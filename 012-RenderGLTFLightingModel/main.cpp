...
public:
	...
	XMVECTOR AmbientLightColor = XMVectorSet(0.1, 0.1, 0.1, 0);		// 环境光的颜色 (亮度)，环境光很微弱
};
...
private:
	...
	std::string ModelFileName = "sakiko/scene.gltf";				// 模型文件名
	std::string ModelTextureFilePath = "sakiko/";					// 模型所在文件夹
...
public:
	...
	void STEP1_InitWindow(HINSTANCE hins)
	{
		...
		// 使用上文的窗口类创建窗口
		m_hwnd = CreateWindow(wc.lpszClassName, L"丰川祥子", WS_SYSMENU | WS_OVERLAPPED,
	...
	void STEP11_AddModelMaterials()
	{
		...
		for (UINT i = 0; i < m_ModelScene->mNumMaterials; i++)
		{
			...
			// 本章教程我们将正式接触光照模型
			// 人能看到颜色，就是因为物体的反射光进入了人眼
			// 光照模型，是根据光学的有关定律，模拟自然界中光照明的物理过程的计算机模型
			// 如何在程序中模拟光照呢？玩游戏的可能听说过光线追踪，但是光线追踪太过复杂，并且需要进行大量计算 (要有高端显卡)
			// 在渲染中，我们将介绍简化过后的模型: Blinn-Phong 光照模型
			
			// 本章我们将学习最简单最基础的 Blinn-Phong 光照模型
			// Bui Tuong Phong 在 1975 年提出了图形学中第一个有影响也是最有影响的光照模型: Phong 光照模型
			// 后经 Jim Blinn 于 1977 年对传统 Phong 光照模型基础上进行改进，提出了 Blinn-Phong 光照模型

			// 光照模型由三部分组成: Ambient 环境光，Diffuse 漫反射光，Specular 镜面光照
			// Ambient 环境光: 光是向着四面八方反弹的，这些从四面八方反弹过来的光能对物体产生间接的影响，这部分称为环境光。
			//				  即使在理论上的黑暗环境中，通常也存在微弱的间接光照 (例如来自月光、远处光源的反射等)，使得物体不会完全黑暗
			//				  为了模拟这种微弱的背景光照，Phong 模型引入了环境光照分量
			//				  它是一个恒定的颜色值，均匀地照亮场景中的所有物体，模拟物体在不受直接光源影响下的基本亮度。
			// 
			// Diffuse 漫反射光: 光打到物体上，会向四面八方反射，这个就叫漫反射，漫反射是影响物体亮度最主要的因素
			//					根据 Lambert Consine Law 兰伯特余弦定理，漫反射分量的大小取决于表面法线和光线的夹角
			//				    入射光的方向和法向量之间的夹角越小越亮，反之越大越暗
			//					这种光照效果使得物体呈现出明显的明暗变化，从而体现出物体的立体感
			// 
			// Specular 高光: 有光泽的物体上面出现的亮点, 模拟高光部分。
			//				 这些亮点是光源在物体表面的反射，其颜色更接近于光源的颜色，而不是物体的固有颜色
			//				 镜面光照的强度取决于观察方向、光线方向和物体表面法线之间的关系
			//				 当观察方向接近光线在物体表面的反射方向时，镜面高光就会变得非常明亮
			//
			// 最终光照 Final Color = Ambient + Diffuse + Specular
	...
	void STEP20_CreatePSO()
	{
		...
		InputElementDesc[2].SemanticName = "TEXCOORD";										// 要锚定的语义
		InputElementDesc[2].SemanticIndex = 0;												// 语义索引
		InputElementDesc[2].Format = DXGI_FORMAT_R32G32_FLOAT;								// 输入格式
		...
		// 顶点自带颜色 float4 color
		InputElementDesc[3].SemanticName = "COLOR";											// 要锚定的语义
		InputElementDesc[3].SemanticIndex = 0;												// 语义索引
		InputElementDesc[3].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;						// 输入格式
		...
		InputElementDesc[4].SemanticName = "BLENDINDICES";									// 要锚定的语义
		InputElementDesc[4].SemanticIndex = 0;												// 语义索引
		InputElementDesc[4].Format = DXGI_FORMAT_R32G32B32A32_UINT;							// 输入格式
		...
		InputElementDesc[5].SemanticName = "BLENDWEIGHT";									// 要锚定的语义
		InputElementDesc[5].SemanticIndex = 0;												// 语义索引
		InputElementDesc[5].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;						// 输入格式
	...
	void Render()
	{
		...
		for (const auto& Mesh : MeshGroup)
		{
			...
			// 设置纹理 SRV (第四个根参数)
			m_CommandList->SetGraphicsRootDescriptorTable(3, MaterialGroup[Mesh.MaterialIndex].GPUHandle);
