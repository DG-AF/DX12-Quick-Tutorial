...
// (14) RenderGLTFNormalMap: 加入纹理贴图，使用 DirectX 12 + Assimp 渲染《舰队 Collection》战列舰大和 gltf 骨骼模型
// 鸣谢原作者大大: 0x384c0 (https://sketchfab.com/0x384c0)
// 模型项目地址: https://sketchfab.com/3d-models/bb-yamato-new-year-mode-addfea2c1142463e814105108e521039
...
public:
	...
	// 获取世界矩阵 (模型矩阵)，用于顶点法线变换
	inline XMMATRIX& GetWorldMatrix()
	{
		return ModelMatrix;
	}

	// 获取摄像机世界空间下的坐标，用于计算光照
...
public:
	...
	XMVECTOR LightDirection = XMVectorSet(1, -1, 1, 0);				// 光源发出的光线方向，默认 (1, -1, 1)
	XMVECTOR LightColor = XMVectorSet(1, 1, 1, 1);					// 光源颜色 (亮度)，默认白色
	XMVECTOR AmbientLightColor = XMVectorSet(1, 1, 1, 0);			// 环境光的颜色 (亮度)，环境光很微弱
};
...
private:
	...
	std::string ModelFileName = "bb_yamato/scene.gltf";				// 模型文件名
	std::string ModelTextureFilePath = "bb_yamato/";				// 模型所在文件夹
	...
	// aiProcess_GenNormals: 对全部网格的顶点，都生成顶点法线 (如果没有法线的话)
	...
	// 纹理结构体，从本章开始材质与纹理分离，一个材质可以对应多副纹理
	struct Texture
	{
		std::string FilePath;						// 材质文件路径
		aiTextureType Type;							// 纹理贴图类型
	...
	// 材质结构体，材质贴图只存储关联的纹理索引与一些光源信息
	struct Material
	{
		UINT TextureIndex = -1;			// 纹理在 TextureGroup 的索引，描述纹理图案/图像信息
		UINT NormalTextureIndex = -1;	// 法线纹理在 TextureGroup 的索引，描述纹理法线信息 (没有用到法线纹理会设置成 -1)

		XMFLOAT4 DiffuseAlbedoLight;	// 贴图的漫反射反照率，表示贴图对光的反射能力
		XMFLOAT4 SpecularLight;			// 贴图的高光反射率，反射高光部分的颜色
		float Glossiness;				// 贴图的光泽度，用于镜面光照
	};

	std::vector<Texture> TextureGroup;				// 模型用到的纹理组
	...
	struct VERTEX									// 顶点数据结构体
	{
		...
		XMFLOAT4 normal;							// 顶点法线，用于光照，同时用于计算 TBN 矩阵
		XMFLOAT4 tangent;							// 顶点切线，切线与法线垂直，用于计算 TBN 矩阵
		XMFLOAT2 texcoordUV;						// 顶点纹理坐标 (法线纹理的 UV 用的也是这个)
	...
	struct MESH										// 网格数据结构体
	{
		UINT MaterialIndex;							// Mesh 用到的纹理材质索引
	...
	ComPtr<ID3D12Resource> m_CBVResource;		// 常量缓冲资源，用于存储 MVP 矩阵，MVP 矩阵每帧都要更新，所以需要存储在常量缓冲区中
	struct CBuffer								// 常量缓冲结构体，设置在 b0 常量缓冲寄存器上，在第一个根参数
	{
	...
	CBuffer* MVPBuffer = nullptr;	// b0 常量缓冲结构体指针，用于 MVP 矩阵与一系列骨骼偏移矩阵



	Light m_GlobalLight;						// 全局光源信息
	ComPtr<ID3D12Resource> m_CBVLightResource;	// 常量缓冲资源，用于存储光照信息
	struct CBuffer_Light						// 用于光源信息的常量缓冲，设置在 b1 常量缓冲寄存器上，在第二个根参数
	{
		XMFLOAT4X4 WorldMatrix;			// 世界矩阵，用于顶点法线变换
		XMFLOAT4 CameraPosition;		// 摄像机在世界空间的坐标

		XMFLOAT4 WorldLightDirection;	// 世界空间上的光线方向 (点光源)
		XMFLOAT4 WorldLightColor;		// 世界空间上的光线颜色 (亮度)
		XMFLOAT4 AmbientLightColor;		// 环境光的颜色 (亮度)
	};
	CBuffer_Light* LightBuffer = nullptr;	// b1 常量缓冲结构体指针，用于一系列光照参数
	...
	D3D12_INPUT_ELEMENT_DESC InputElementDesc[7] = {};			// 输入元素信息结构体数组
	D3D12_GRAPHICS_PIPELINE_STATE_DESC PSODesc = {};			// PSO 信息结构体
	D3D12_GRAPHICS_PIPELINE_STATE_DESC NormalMapPSODesc = {};	// NormalMapPSO 的 PSO 信息结构体
	ComPtr<ID3D12PipelineState> m_NonNormalMapPSO;				// 未使用法线贴图的 PSO
	ComPtr<ID3D12PipelineState> m_NormalMapPSO;					// 使用了法线贴图的 PSO

	ComPtr<ID3DBlob> VertexShaderBlob;		// 顶点着色器二进制字节码
	ComPtr<ID3DBlob> PixelShaderBlob;		// 像素着色器二进制字节码
	ComPtr<ID3DBlob> ErrorBlob;				// 错误字节码，根签名创建失败时用 ErrorBlob->GetBufferPointer(); 可以获取报错信息


	// 为了更好的对比法线贴图的效果，我们设置了这个变量来决定使用的是 NonNormalMapPSO 或 NormalMapPSO
	// 按空格可以改变渲染状态，为 true 时带有法线贴图的材质会计算并使用法线贴图，为 false 则全部按普通纹理渲染，启动程序后默认不开启
	bool IsUseNormalMapRender = false;
...
public:
	...
	void STEP1_InitWindow(HINSTANCE hins)
	{
		...
		// 使用上文的窗口类创建窗口
		m_hwnd = CreateWindow(wc.lpszClassName, L"大和 —— 未使用法线纹理渲染", WS_SYSMENU | WS_OVERLAPPED,
	...
	{
		// 使用 ReadFile 函数直接传递文件路径打开文件，注意路径不能有中文字符
	...
	void STEP11_AddModelMaterials()
	{
		...
		for (UINT i = 0; i < m_ModelScene->mNumMaterials; i++)
		{
			...
			Material mt = {};						// 新材质，这里改了前面的设计，现在材质与纹理分离了
			...
			{
				Texture tex = {};						// 新纹理
				tex.Type = aiTextureType_EMISSIVE;		// 新材质的类型
				tex.FilePath = ModelTextureFilePath;	// 新材质所在的文件夹
				tex.FilePath += texturePath.C_Str();	// 新材质的文件名
				TextureGroup.push_back(tex);			// 纹理组添加新纹理

				MaterialGroup[i].TextureIndex = TextureGroup.size() - 1;	// 新材质设置纹理
			}
			...
			else if (material->GetTexture(aiTextureType_DIFFUSE, 0, &texturePath) == aiReturn_SUCCESS)
			{
				Texture tex = {};						// 新纹理
				tex.Type = aiTextureType_DIFFUSE;		// 新材质的类型
				tex.FilePath = ModelTextureFilePath;	// 新材质所在的文件夹
				tex.FilePath += texturePath.C_Str();	// 新材质的文件名
				TextureGroup.push_back(tex);			// 纹理组添加新纹理
			...
			{
				Texture tex = {};						// 新纹理
				tex.Type = aiTextureType_NONE;			// 新材质的类型
				TextureGroup.push_back(tex);			// 纹理组添加新纹理
			...
			// 接下来检测贴图是否有对应的法线贴图 (默认纹理肯定是没有法线贴图的，这一点不必担心，后面的代码会处理)
			// 如你所见，有些材质不仅包含漫反射/自发光贴图，还会包含一副对应的纹理贴图
			// 图形学先驱们发现：逐像素插值法线仍然是心有余而力不足，物体的法线像是直接"固定"在表面上，看起来像是贴图糊上去的
			// 漫反射效应反而很弱，摄像机换个特定角度就能看出 3D 物体的破绽了，不真实。
			// 现实世界的粗糙物体，表面都是粗糙不平的，由于各种原因，表面法线随机分布 (原本垂直于表面的法线，受到了随机的扰动)
			// 表面越是粗糙不平，随机扰动越大，它的漫反射效应会更加剧烈一点
			// 这些随机反射出来的光线"欺骗"了人眼，让人眼觉得这个物体表面更加凹凸不平
			// 为了在不增加额外顶点的情况下，模拟一些物体表面粗糙不平的效果，营造更强的既视感，增加更多细节，法线贴图应运而生。
			// 法线贴图每个像素存储的是 切线空间下物体表面的法向量 压缩后的数据 (r,g,b)
			// r -> 与纹理水平方向对齐 -> tangent 切线方向的分量
			// g -> 与纹理垂直方向对齐 -> bitangent 副切线方向的分量
			// b -> 与宏观三角形表面的法线方向对齐 -> normal 法线方向的分量
			// 在这个坐标系下，一个完全垂直于宏观表面、未受扰动的微观法线的方向向量是 (0, 0, 1)
			// 法线很多时候都是受到轻微的扰动，不会偏离垂直中心太远，很多时候分量 b 都是靠近 1 的，所以法线纹理呈蓝紫色
			// 从纹理中取出并解压这些数据，利用 TBN 矩阵变换到模型空间并加入光照，能使物体表面更逼真
			if (material->GetTexture(aiTextureType_NORMALS, 0, &texturePath) == aiReturn_SUCCESS)
			{
				Texture tex = {};						// 新纹理
				tex.Type = aiTextureType_NORMALS;		// 新材质的类型
				tex.FilePath = ModelTextureFilePath;	// 新材质所在的文件夹
				tex.FilePath += texturePath.C_Str();	// 新材质的文件名
				TextureGroup.push_back(tex);			// 纹理组添加新纹理

				MaterialGroup[i].NormalTextureIndex = TextureGroup.size() - 1;	// 新材质设置法线纹理
			}


			// 从每张贴图中获取 DiffuseAlbedo 分量，此分量表示该贴图的反照率 (对光的反射能力)
			// DiffuseMap 不仅包含颜色信息，还包含了光影信息 (Albedo 反射率分量)，用于调整物体表面色彩
			// RGB 三种分量表示对红、绿、蓝三色的反射程度 (权重)，权重越大，越接近纹理原色
			aiColor4D DiffuseAlbedo = {};
			// 如果贴图有反照信息，就获取，注意 AI_MATKEY_COLOR_DIFFUSE 是一个宏，包含了三个参数
			if (aiGetMaterialColor(material, AI_MATKEY_COLOR_DIFFUSE, &DiffuseAlbedo) == aiReturn_SUCCESS)
			{
				MaterialGroup[i].DiffuseAlbedoLight =
					XMFLOAT4(DiffuseAlbedo.r, DiffuseAlbedo.g, DiffuseAlbedo.b, DiffuseAlbedo.a);
			}
			else // 没有，就都设置为 1
			{
				MaterialGroup[i].DiffuseAlbedoLight = XMFLOAT4(1, 1, 1, 1);
			}


			// 从每张贴图获取 Specular 高光分量，此分量表示贴图反射高光产生的颜色
			// RGB 分量表示反射高光包含红、绿、蓝三色的权重，权重越大，越接近入射光线
			aiColor4D Specular = {};
			// 如果贴图有高光信息，就获取，注意 AI_MATKEY_COLOR_SPECULAR 是一个宏，包含了三个参数
			if (aiGetMaterialColor(material, AI_MATKEY_COLOR_SPECULAR, &Specular) == aiReturn_SUCCESS)
			{
				MaterialGroup[i].SpecularLight =
					XMFLOAT4(Specular.r, Specular.g, Specular.b, Specular.a);
			}
			else // 没有，就都设置成 1
			{
				MaterialGroup[i].SpecularLight = XMFLOAT4(1, 1, 1, 1);
			}


			// 从每张贴图获取光泽度 (粗糙度)， Glossiness (Shininess/Roughness) 光泽度表示物体表面的光滑程度
			// 光滑的物体通常更容易发生镜面反射，会反射出清晰的光斑，光泽度没有上限，越高越光滑，反射高光越接近入射光线
			// 而粗糙的物体通常更容易发生漫反射，更容易发生散射现象，光斑会更模糊直到消失，0 表示完全漫反射
			float Glossiness = 0;
			// 如果贴图有光泽度，就获取，注意 AI_MATKEY_COLOR_SHININESS 是一个宏，包含了三个参数
			if (aiGetMaterialFloat(material, AI_MATKEY_SHININESS, &Glossiness) == aiReturn_SUCCESS)
			{
				MaterialGroup[i].Glossiness = Glossiness;
			}
		}
	}
	...
	void STEP12_AddModelData()
	{
		...
		for (UINT i = 0; i < m_ModelScene->mNumMeshes; i++)
		{
			...
			for (UINT j = 0; j < mesh->mNumVertices; j++)
			{
				...
				// 新顶点法线坐标，用于计算光照，同时计算 TBN 矩阵 (TBN 矩阵的说明在 shader.hlsl)
				new_vertex.normal = XMFLOAT4(mesh->mNormals[j].x, mesh->mNormals[j].y, mesh->mNormals[j].z, 0);

				// 如果顶点还有切线，就添加，用于计算光照与 TBN 矩阵 (关联到法线纹理贴图的 mesh 顶点才有切线)
				if (mesh->HasTangentsAndBitangents())
				{
					new_vertex.tangent = XMFLOAT4(mesh->mTangents[j].x, mesh->mTangents[j].y, mesh->mTangents[j].z, 0);
				}
				...
				// 大部分 mesh 法线纹理和其他纹理一样是共用同一个 UV 通道的 (包括我们这个模型)，在 shader 上面只需要用同一个 UV 采样就行
				// 具体还是要看建模师怎么设置的。少数纹理使用的是不同 UV 通道，如果用到的是不同 UV 通道
				// aiMesh::GetNumTextureCoordsName 和 aiMesh::GetNumUVChannels 可以检测到，我们基本不会涉及到这种情况，不用担心
			...
			if (mesh->HasBones())
			{
				...
				for (UINT j = 0; j < mesh->mNumBones; j++)
				{
					...
					// BoneNode_IndexGroup 添加新键值对 (Mesh Space -> Model Space)
	...
	void CreateSRVHeap()
	{
		...
		SRVHeapDesc.NumDescriptors = TextureGroup.size();					// 描述符堆的容量，注意这里是纹理组的长度，不是材质组的！
	...
	void CreateDefaultTexture(const UINT index)
	{
		...
		m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE, &UploadResourceDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&TextureGroup[index].UploadTexture));
		...
		m_D3D12Device->CreateCommittedResource(&DefaultHeapDesc, D3D12_HEAP_FLAG_NONE, &DefaultResourceDesc,
			D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&TextureGroup[index].DefaultTexture));
	}
	...
	void CopyDefaultTextureToDefaultResource(const UINT index)
	{
		...
		TextureGroup[index].UploadTexture->Map(0, nullptr, reinterpret_cast<void**>(&TransferPointer));
		...
		TextureGroup[index].UploadTexture->Unmap(0, nullptr);

		D3D12_PLACED_SUBRESOURCE_FOOTPRINT PlacedFootprint = {};									// 资源脚本，用来描述要复制的资源
		D3D12_RESOURCE_DESC DefaultResourceDesc = TextureGroup[index].DefaultTexture->GetDesc();	// 默认堆资源结构体
		...
		DstLocation.pResource = TextureGroup[index].DefaultTexture.Get();	// 要复制到的资源
		...
		SrcLocation.pResource = TextureGroup[index].UploadTexture.Get();	// 被复制数据的缓冲
	...
	void CreateSRV(const UINT index, D3D12_CPU_DESCRIPTOR_HANDLE CPUHandle, D3D12_GPU_DESCRIPTOR_HANDLE GPUHandle)
	{
		...
		m_D3D12Device->CreateShaderResourceView(TextureGroup[index].DefaultTexture.Get(), &SRVDescriptorDesc, CPUHandle);

		// 将当前 SRV 描述符句柄存储到 ModelManager 的纹理映射表中，注意我们传的是引用参数，可以直接对参数进行修改
		TextureGroup[index].CPUHandle = CPUHandle;
		TextureGroup[index].GPUHandle = GPUHandle;
	}
	...
	void STEP14_CreateModelTextureResource()
	{
		...
		{
			// 如果不是默认纹理
			if (TextureGroup[i].Type != aiTextureType_NONE)
			{
				// 从纹理文件中加载数据
				LoadTextureFromFile(StringConvertToWString(TextureGroup[i].FilePath));
				// 创建上传堆和默认堆资源
	...
	// 创建 Constant Buffer Resource 常量缓冲资源，常量缓冲是一块预先分配的高速显存
	void STEP18_CreateCBVResource()
	{
		...
		// 常量缓冲直接 Map 映射到结构体指针就行即可，无需 Unmap 关闭映射
		m_CBVResource->Map(0, nullptr, reinterpret_cast<void**>(&MVPBuffer));



		// GlobalLight 全局光照常量资源的大小
		UINT GlobalLightWidth = Ceil(sizeof(CBuffer_Light), 256) * 256;

		// 这里我们复用上面的 CBVResourceDesc，改一下宽度就可以创建资源了
		CBVResourceDesc.Width = GlobalLightWidth;

		// 创建常量缓冲资源
		m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE, &CBVResourceDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_CBVLightResource));

		// 开启映射，和上文同理
		m_CBVLightResource->Map(0, nullptr, reinterpret_cast<void**>(&LightBuffer));
	}
	...
	void STEP19_CreateRootSignature()
	{
		...
		D3D12_ROOT_PARAMETER RootParameters[4] = {};						// 根参数数组
		...
		D3D12_ROOT_DESCRIPTOR CBVRootDescriptorDesc = {};					// 常量缓冲根描述符信息结构体 (b0)
		...
		D3D12_ROOT_DESCRIPTOR CBVRootDescriptor2Desc = {};					// 常量缓冲根描述符信息结构体 (b1)
		CBVRootDescriptor2Desc.ShaderRegister = 1;							// 要绑定的寄存器编号，这里对应 HLSL 的 b1 寄存器
		CBVRootDescriptor2Desc.RegisterSpace = 0;							// 要绑定的命名空间，这里对应 HLSL 的 space0

		RootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;	// 常量缓冲对整个渲染管线都可见
		RootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;	// 根参数的类型：CBV 根描述符
		RootParameters[1].Descriptor = CBVRootDescriptor2Desc;				// 填上文的结构体



		// 第三个根参数: Root Constants 根常量，根常量设置很方便，间接寻址开销为 0，无需绑定缓冲资源和描述符/描述符堆
		// 但根常量消耗巨大，一个 32 位 (4 Byte) 根常量要占 1 DWORD

		// 我们这里使用根常量，是因为 CommandQueue 要到 ExecuteCommandList 才执行命令
		// 在 CommandList 添加命令的时候，按材质索引更新光照信息，GPU 还没启动，这时候按索引更新没什么用
		// 如果只是将材质光照信息放到上面的常量缓冲 (绑定到根描述符)，每帧总是只会索引到最后一张材质的信息，其他材质形同虚设
		// 如何才能修改命令也一并按顺序放入命令队列中呢? 根参数就可以帮我们解决这个问题

		D3D12_ROOT_CONSTANTS CBVRootConstantsDesc = {};		// 根常量信息结构体 (b2)
		CBVRootConstantsDesc.Num32BitValues = 9;			// 32 位根常量的个数，我们要用 9 个 32 位 float
		...
		// 第四个根参数：根描述表 (Range: SRV)
		// 根描述表类似于生活中的多位插座 (排插)，插座可以一次连接多个电器，根描述表也能一次绑定多个描述符
		// 但它的缺点是: 如果要绑定多个描述符，描述符在堆上必须按特定的顺序 (根描述符指定的顺序) 排列，你写了什么，描述符就得怎么放，这样牺牲了灵活性
		// 我们在 PS shader 上要同时使用 diffuse 纹理与法线纹理，在 BaseShaderRegister 上需要指定为 2
		// 调用 SetGraphicsRootDescriptorTable，描述符堆上两个连续的描述符就会被分别引用到 t0 和 t1
		// 如果材质还会有法线纹理，在堆上 diffuse 纹理和法线纹理是紧挨放在一起的；如果没有 t1 也不会用到 (后文和 shader 会讲到)
		// 你可能会担心引用越界问题 (t1 引用到描述符外面)，这一点不用担心，因为我们的设计不会发生这种情况的
		...
		SRVDescriptorRangeDesc.NumDescriptors = 2;								// Range 里面的描述符数量 N，一次可以绑定多个描述符到多个寄存器槽上
		...
		rootsignatureDesc.NumParameters = 4;						// 根参数数量
	...
	// 创建没有使用法线贴图的 PSO
	void STEP20_CreateNonNormalMapPSO()
	{
		...
		// 顶点法线 float4 normal
		InputElementDesc[1].SemanticName = "NORMAL";										// 要锚定的语义
		...
		// 顶点切线 float4 tangent
		InputElementDesc[2].SemanticName = "TANGENT";										// 要锚定的语义
		...
		InputElementDesc[2].AlignedByteOffset = 32;											// 在输入槽中的偏移
		...
		InputElementDesc[3].SemanticName = "TEXCOORD";										// 要锚定的语义
		InputElementDesc[3].SemanticIndex = 0;												// 语义索引
		InputElementDesc[3].Format = DXGI_FORMAT_R32G32_FLOAT;								// 输入格式
		InputElementDesc[3].InputSlot = 0;													// 输入槽编号
		InputElementDesc[3].AlignedByteOffset = 48;											// 在输入槽中的偏移
		...
		// 顶点自带颜色 float4 color
		InputElementDesc[4].SemanticName = "COLOR";											// 要锚定的语义
		...
		InputElementDesc[5].SemanticName = "BLENDINDICES";									// 要锚定的语义
		InputElementDesc[5].SemanticIndex = 0;												// 语义索引
		InputElementDesc[5].Format = DXGI_FORMAT_R32G32B32A32_UINT;							// 输入格式
		InputElementDesc[5].InputSlot = 0;													// 输入槽编号
		InputElementDesc[5].AlignedByteOffset = 72;											// 在输入槽中的偏移
		InputElementDesc[5].InputSlotClass = D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA;	// 输入流类型
		InputElementDesc[5].InstanceDataStepRate = 0;										// 实例数据步进率


		// 骨骼权重 float4 BoneWeights
		InputElementDesc[6].SemanticName = "BLENDWEIGHT";									// 要锚定的语义
		InputElementDesc[6].SemanticIndex = 0;												// 语义索引
		InputElementDesc[6].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;						// 输入格式
		InputElementDesc[6].InputSlot = 0;													// 输入槽编号
		InputElementDesc[6].AlignedByteOffset = 88;											// 在输入槽中的偏移
		InputElementDesc[6].InputSlotClass = D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA;	// 输入流类型
		InputElementDesc[6].InstanceDataStepRate = 0;										// 实例数据步进率



		InputLayoutDesc.NumElements = 7;						// 输入元素个数
		...
		// 最终创建 PSO 对象
		m_D3D12Device->CreateGraphicsPipelineState(&PSODesc, IID_PPV_ARGS(&m_NonNormalMapPSO));

	}


	// 创建有使用法线贴图的 PSO，复用 NonNormalMapPSO 的结构体，增加不同的选项
	void STEP21_CreateNormalMapPSO()
	{
		// 复用 PSO 结构体
		NormalMapPSODesc = PSODesc;

		// shader 预编译需要用到的宏定义，HLSL 也可以像 C/C++ 一样使用宏定义和宏控制语句，但是要使用这些宏定义就需要在外部控制
		// HLSL 预编译时 (D3DCompileFromFile)，会将参数输入到命令行中，编译程序接收到参数，就会将 HLSL 内的宏定义语句全部替换和去除
		// HLSL 的宏定义类似于 C/C++ 中的 #define Name Definition
		D3D_SHADER_MACRO NormalMapMacro[2] =
		{
			{"USE_NORMALMAP", 0},	// 相当于 #define USE_NORMALMAP 0
			{NULL, NULL}			// 终止符，类似字符串的 '\0'，D3DCompileFromFile 不会接收结构体长度参数，碰到终止符就结束，所以需要我们手动定义终止符
		};

		// 再次编译像素着色器 Pixel Shader，这次的 Shader Blob 带上宏定义 (第二个参数) 给 NormalMapPSO
		D3DCompileFromFile(L"shader.hlsl", NormalMapMacro, nullptr, "PSMain", "ps_5_1", NULL, NULL, &PixelShaderBlob, &ErrorBlob);
		if (ErrorBlob)		// 如果着色器编译出错，ErrorBlob 可以提供报错信息
		{
			OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
			OutputDebugStringA("\n");
		...
		NormalMapPSODesc.PS.pShaderBytecode = PixelShaderBlob->GetBufferPointer();
		NormalMapPSODesc.PS.BytecodeLength = PixelShaderBlob->GetBufferSize();

		NormalMapPSODesc.BlendState.RenderTarget[0].BlendEnable = false;

		// 最终创建 PSO 对象
		m_D3D12Device->CreateGraphicsPipelineState(&NormalMapPSODesc, IID_PPV_ARGS(&m_NormalMapPSO));
	}
	...
	// 这里还会更新全局光照缓冲 (m_GlobalLight) 的信息
	void UpdateConstantBuffer()
	{
		...
		// 更新全局光照缓冲的世界矩阵
		XMStoreFloat4x4(&LightBuffer->WorldMatrix, m_FirstCamera.GetWorldMatrix());
		// 更新摄像机位置
		XMStoreFloat4(&LightBuffer->CameraPosition, m_FirstCamera.GetEyePosition());

		// 每帧更新光线方向之前，对光线按世界空间下的 y 轴进行旋转，就是这里进行了光源选择
		// XMVector3Transform 将向量与矩阵相乘 (前三个分量)，实现向量的各种变换
		m_GlobalLight.LightDirection =
			XMVector3Transform(m_GlobalLight.LightDirection, XMMatrixRotationY(0.01 * XM_PIDIV2));

		// 更新光源颜色
		XMStoreFloat4(&LightBuffer->WorldLightColor, m_GlobalLight.LightColor);
		// 更新光线 (平行光) 方向
		XMStoreFloat4(&LightBuffer->WorldLightDirection, m_GlobalLight.LightDirection);
		// 更新环境光
		XMStoreFloat4(&LightBuffer->AmbientLightColor, m_GlobalLight.AmbientLightColor);
	}
	...
	void Render()
	{
		...
		// 设置常量缓冲 (第二个根参数)，本次我们要设置全局光照信息
		m_CommandList->SetGraphicsRootConstantBufferView(1, m_CBVLightResource->GetGPUVirtualAddress());
		...
		{
			// 设置根常量 (第三个根参数)，设置根参数的命令，会跟随 CommandList 一起送入 CommandQueue 中
			// 这样就实现了 "画什么材质就用到什么光照数据"，绘制不同材质的 Mesh 时光照数据的同步
			// 根常量可以一次连续设置 (只要 CPU 和 GPU 对应的变量在内存分布上是连续的就行)
			// 我们在 cpp 和 shader 端特意设置 DiffuseAlbedoLight, SpecularLight, Glossiness 从上到下在一起
			// 这样它们的内存分布就连续了，我们就可以用 SetGraphicsRoot32BitConstants 一次性设置了，相当方便
			m_CommandList->SetGraphicsRoot32BitConstants(2, 9,
				static_cast<void*>(&MaterialGroup[Mesh.MaterialIndex].DiffuseAlbedoLight), 0);

			// 设置纹理 SRV (第四个根参数)，注意这里会同时设置两个描述符到 t0 和 t1
			m_CommandList->SetGraphicsRootDescriptorTable(3, TextureGroup[MaterialGroup[Mesh.MaterialIndex].TextureIndex].GPUHandle);

			
			// 设置渲染管线，有时候频繁切换 PSO 是一个开销很大的操作，不建议像我这样干，读者可自行考虑如何优化
			if (IsUseNormalMapRender && MaterialGroup[Mesh.MaterialIndex].NormalTextureIndex != -1)
			{
				// 如果开启了渲染选项，并且 mesh 使用了法线贴图，切换成 NormalMapPSO
				m_CommandList->SetPipelineState(m_NormalMapPSO.Get());
			}
			else
			{
				// 正常渲染，不使用纹理贴图
				m_CommandList->SetPipelineState(m_NonNormalMapPSO.Get());
			}
		...
		switch (msg)
		{
			...
			case WM_CHAR:	// 获取键盘产生的字符消息，TranslateMessage 会将虚拟键码翻译成字符码，同时产生 WM_CHAR 消息
			{
				switch (wParam)		// wParam 是按键对应的字符 ASCII 码
				{
					...
					case ' ':	// 空格
						IsUseNormalMapRender = !IsUseNormalMapRender;

						if (IsUseNormalMapRender)
						{
							SetWindowText(hwnd, L"大和 —— 使用法线纹理渲染");
						}
						else
						{
							SetWindowText(hwnd, L"大和 —— 已使用法线纹理渲染");
						}
	...
	static void Run(HINSTANCE hins)
	{
		...
		engine.STEP20_CreateNonNormalMapPSO();
		engine.STEP21_CreateNormalMapPSO();

		engine.STEP22_RenderLoop();
	}
};
