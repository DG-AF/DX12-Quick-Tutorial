...
// 下面这种是另一种写法，但我们并没有使用它，想想为什么？

struct VSInput      // VS 阶段输入顶点数据
{
	float4 position : POSITION;			// 输入顶点的位置
	float4 normal : NORMAL;				// 顶点法线，用于光照与 TBN 矩阵计算
	float4 tangent : TANGENT;			// 顶点切线，用于光照与 TBN 矩阵计算
...
VSOutput VSMain(VSInput input)
{
	...
	// 切线与法线垂直，切线一样要做上述变换
	output.tangent = mul(input.tangent, BoneMatrix);	// 先做骨骼变换
	output.tangent = mul(output.tangent, WorldMatrix);	// 再做世界变换
...
Texture2D m_DiffuseMap : register(t0, space0);	// 漫反射/自发光纹理
Texture2D m_NormalMap : register(t1, space0);	// 法线纹理
...
{
#ifdef USE_NORMALMAP
	return m_DiffuseMap.Sample(m_sampler, input.texcoordUV);
#endif
	...
	// 法线光栅化阶段会进行逐像素插值，因为前面的计算可能会有浮点舍入误差，使用前要先进行单位化
	float3 Normal = normalize(input.normal.xyz);
	
	
	// ---------------------------------------------------------------------------------------------------------------
	
	// 如果在 cpp 端传递了 USE_NORMALMAP 这个宏定义 (切换到 NormalMapPSO)，渲染法线贴图，否则 HLSL 编译器将忽略这段代码
	// 在 PS 上计算 TBN 矩阵，然后将纹理的法线从切线空间变换到模型空间，想一想我们为什么没有使用这种写法？
	...
	// 构建 TBN 矩阵，将切线空间下的法向量变换到模型空间
	float3x3 TBNMatrix = float3x3(Tangent, Bitangent, Normal);

	// 采样法线贴图，并将颜色值从 [0,1] 转换到 [-1,1]
	// 公式: normal * 2 - 1;
	float3 TangentSpaceNormal = m_NormalMap.Sample(m_sampler, input.texcoordUV);
	TangentSpaceNormal = normalize(TangentSpaceNormal * 2.0 - 1.0);
	
	// 将法线与 TBN 矩阵相乘，得到最终用于光照的法线
	Normal = normalize(mul(Normal.xyz, TBNMatrix));
