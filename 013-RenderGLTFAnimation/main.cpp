...
private:
	...
	UINT ModelImportFlag = aiProcess_ConvertToLeftHanded | aiProcess_Triangulate |
		...
		aiProcess_GenBoundingBoxes | aiProcess_JoinIdenticalVertices | aiProcess_PopulateArmatureData;
...
public:
	...
	void STEP13_CalcModelBoundingBox()
	{
		...
		*/
		...
		// 设置摄像机焦点
		m_FirstCamera.SetFocusPosition(XMVectorSet(0, 0, 0, 1.0));

		// 设置摄像机位置
		m_FirstCamera.SetEyePosition(XMVectorSet(2, 3, -3, 1.0));
	}
	...
	void BeginAnimationRender()
	{
		if (AnimationIndex != 0)	// 如果不是 Static Pose，就渲染
		{
			...
			// 更换动画，更新标题
			std::string WindowTitle = "绀紫之心 -- 动画(" + std::to_string(AnimationIndex) + "): " +
				m_ModelScene->mAnimations[AnimationIndex - 1]->mName.C_Str();
			SetWindowTextA(m_hwnd, WindowTitle.c_str());
		}
		else
		{
			SetWindowText(m_hwnd, L"绀紫之心 -- Static Pose 静态姿态");
		}
	}
