...
// 目的是希望大家重点关注新东西：命令签名、ExecuteIndirect 间接命令绘制与 GPU 视锥剔除
class D2DEngine
{
private:
...
private:
	...
	// 最大区块加载半径是 8 (包括玩家所处区块本身)，进入加载半径的新区块就加载，加载范围 (9+9-1)^2 = 289
	const int MaxCreateRadius = 9;
	// 最大区块保持半径是 9 (包括玩家所处区块本身)，脱离保持半径的区块将被卸载，保持范围 (10+10-1)^2 = 361
	const int MaxUnloadRadius = 10;
	// 16x16 区块边长
	...
	// UAV 纹理数组和 SRV 待加载列表一次创建的最大区块数量 (17x17=289，最大加载范围)
	const UINT MaxCreateChunkCount = (MaxCreateRadius * 2 - 1) * (MaxCreateRadius * 2 - 1);
	// 实例缓冲所能容纳的最大已加载区块数量 (19x19=361，最大保持范围)
	...
	// 16x16x24 的区块元信息结构体，存储区块 实例偏移、实例量、区块状态、AABB 包围盒 这些元信息
	struct CHUNK
	{
		...
		BoundingBox AABBBox = {};	// 整个区块的 AABB 包围盒，用于 GPU 视锥剔除
	};


	// 所有 16x16x24 区块的半边长
	const XMFLOAT3 ChunkExtents = { ChunkSideLength / 2.0f, MaxChunkHeight / 2.0f, ChunkSideLength / 2.0f };


	// 用于放置区块实例元信息的 HiveBuffer 缓冲，它不是连续缓冲，里面有数据空位 (最大 361 个区块元素)
	...
	// 间接命令签名，用于描述 GPU 驱动的命令格式与布局
	ComPtr<ID3D12CommandSignature> m_CommandSignature;


	// CPU 向 GPU 上传所有所有已加载区块元数据的 SRV 结构化缓冲上传堆资源
	// StructuredBuffer<CHUNK> m_KeepChunksBuffer : register(t0, space0);
	ComPtr<ID3D12Resource> m_SRVKeepChunksBuffer_UploadResource;

	// 存储间接绘制命令，由 GPU 驱动、解析并执行的 UAV 结构化缓冲默认堆资源
	// RWStructuredBuffer<D3D12_DRAW_INDEXED_ARGUMENTS> m_IndirectCommandBuffer : register(u0, space0);
	ComPtr<ID3D12Resource> m_UAVIndirectCommandBuffer_DefaultResource;

	// 可见区块 (间接绘制命令) 计数器 UAV 原始缓冲区默认堆资源，清零同样由计算着色器执行
	// RWByteAddressBuffer m_IndirectCommandCounter : register(u1, space0);
	ComPtr<ID3D12Resource> m_UAVIndirectCommandCounter_DefaultResource;


	// CullingShader 常量缓冲的结构体
	struct CullingCBuffer
	{
		XMFLOAT4 FrustumPlanes[6];	// 组成视锥体的六个平面方程 (ax, by, cz, d)，法线指向视锥体内部
		UINT KeepChunkCount;		// 已加载 (处于保持状态) 的区块数量
	};

	// 指向 Culling 常量缓冲资源的结构体指针
	CullingCBuffer* CullingCbufferPointer = nullptr;


	// 存储 6 个视锥平面方程系数 和 已记载区块区块数量 的 CBV 常量缓冲上传堆资源
	// cbuffer CSGlobalData : register(b0, space0);
	ComPtr<ID3D12Resource> m_CBVCullingGlobalBuffer_UploadResource;


	// 指向 m_SRVKeepChunksBuffer_UploadResource，上传待进行 GPU Culling 区块的指针
	BYTE* CullingTestUploadPointer = nullptr;
	...
	// CullingShader 使用的根签名
	ComPtr<ID3D12RootSignature> m_CullingRootSignature;
	// CullingShader 使用的 PSO
	ComPtr<ID3D12PipelineState> m_CullingPSO;
	// 用于清空命令计数器的 PSO
	ComPtr<ID3D12PipelineState> m_ClearCounterPSO;


	// 将 CommandBuffer 和 Counter 从 UNORDERED_ACCESS -> INDIRECT 的两个资源屏障
	D3D12_RESOURCE_BARRIER UAVToIndirect_barrier[2] = {};
...
public:
	...
	// 创建实例流缓冲资源，最大能容纳 361 个 16x16x24 的区块，用的是 VBV1
	...
	void STEP29_InitChunkContainer()
	{
		...
		// 区块缓冲先推入 361 个空泡，之后大小就固定了，不会再变了
	...
	// 创建 CommandSignature 命令签名，将渲染的指挥权交给 GPU，这样就能和 GPU Culling 无缝衔接
	void STEP30_CreateCommandSignature()
	{
		// CommandSignature 的参数列表
		// Para 0: DrawIndexedInstanced 绘制命令


		// 命令参数结构体，定义了每一批命令的格式
		D3D12_INDIRECT_ARGUMENT_DESC CommandParameters = {};

		// 第一个命令参数: DrawIndexedInstanced
		CommandParameters.Type = D3D12_INDIRECT_ARGUMENT_TYPE_DRAW_INDEXED;


		// 命令签名信息结构体
		D3D12_COMMAND_SIGNATURE_DESC CommandSignatureDesc = {};
		// 每一批命令的实际字节数 (D3D12_DRAW_INDEXED_ARGUMENTS = 5*4 = 20 Bytes)
		CommandSignatureDesc.ByteStride = sizeof(D3D12_DRAW_INDEXED_ARGUMENTS);
		// 命令参数的个数 (只有一个命令参数)
		CommandSignatureDesc.NumArgumentDescs = 1;
		// 指向命令参数结构体的指针
		CommandSignatureDesc.pArgumentDescs = &CommandParameters;


		// 创建命令签名
		// 第二个参数是要关联的根签名
		// 命令签名还可以让 GPU 来设置需要的 CBV/SRV/UAV 根描述符，根常量，甚至还可以驱动计算着色器
		// 如果命令签名中包含了需要绑定根参数的字段 (根描述符，根常量)，则必须提供一个根签名指针，
		// 以便驱动验证命令中引用的根参数索引是否有效，我们这里不需要，直接 nullptr
		m_D3D12Device->CreateCommandSignature(&CommandSignatureDesc, nullptr,
			IID_PPV_ARGS(&m_CommandSignature));
	}


	// 创建 CullingShader 进行 GPU 视锥剔除 (以及计数器清零) 所需要的各种资源
	void STEP31_CreateCullingRequireResource()
	{
		// 创建 m_SRVKeepChunksBuffer_UploadResource
		{
			// 上传堆资源结构体信息，注意资源宽度!
			D3D12_RESOURCE_DESC UploadResourceDesc = {};
			UploadResourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
			UploadResourceDesc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
			UploadResourceDesc.Width = MaxKeepChunkCount * sizeof(CHUNK);
			UploadResourceDesc.Height = 1;
			UploadResourceDesc.Format = DXGI_FORMAT_UNKNOWN;
			UploadResourceDesc.DepthOrArraySize = 1;
			UploadResourceDesc.MipLevels = 1;
			...
			m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE,
				&UploadResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
				IID_PPV_ARGS(&m_SRVKeepChunksBuffer_UploadResource));


			// 进行持续化映射，通过指针进行写入操作，在 GPU 进行读取之前都能完成
			m_SRVKeepChunksBuffer_UploadResource->Map(0, nullptr,
				reinterpret_cast<void**>(&CullingTestUploadPointer));
		}


		// 创建 m_UAVIndirectCommandBuffer_DefaultResource
		{
			// 默认堆资源结构体信息，注意资源宽度和 Flag!
			D3D12_RESOURCE_DESC DefaultResourceDesc = {};
			DefaultResourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
			DefaultResourceDesc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
			DefaultResourceDesc.Width = MaxKeepChunkCount * sizeof(D3D12_DRAW_INDEXED_ARGUMENTS);
			...
			m_D3D12Device->CreateCommittedResource(&DefaultHeapDesc, D3D12_HEAP_FLAG_NONE,
				&DefaultResourceDesc, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
				IID_PPV_ARGS(&m_UAVIndirectCommandBuffer_DefaultResource));
		}


		// 创建 m_UAVIndirectCommandCounter_DefaultResource
		{
			// 默认堆资源结构体信息，注意资源宽度和 Flag!
			// 原始缓冲区长度必须是 4 的倍数
			D3D12_RESOURCE_DESC DefaultResourceDesc = {};
			...
			m_D3D12Device->CreateCommittedResource(&DefaultHeapDesc, D3D12_HEAP_FLAG_NONE,
				&DefaultResourceDesc, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, nullptr,
				IID_PPV_ARGS(&m_UAVIndirectCommandCounter_DefaultResource));
		}


		// 创建 m_CBVCullingGlobalBuffer_UploadResource
		{
			// 常量资源宽度，这里填整个结构体的大小，注意 256 字节对齐！
			// D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT = 256
			UINT CBufferWidth = Ceil(sizeof(CullingCBuffer), 256) * 256;
			...
			UploadResourceDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
			UploadResourceDesc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
			UploadResourceDesc.Width = CBufferWidth;
			UploadResourceDesc.Height = 1;
			UploadResourceDesc.Format = DXGI_FORMAT_UNKNOWN;
			UploadResourceDesc.DepthOrArraySize = 1;
			UploadResourceDesc.MipLevels = 1;
			...
			m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE,
				&UploadResourceDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
				IID_PPV_ARGS(&m_CBVCullingGlobalBuffer_UploadResource));
	...
	// 创建专门用于 CullingShader 的根签名
	void STEP32_CreateCullingRootSignature()
	{
		// NoiseRootSignature 的根参数 + 静态采样器列表
		// Para 0: (Type = Root Descriptor,	 2 DWORD)  (t0, space0) SRV 根描述符，用于已加载区块元信息缓冲
		// Para 1: (Type = Root Descriptor,	 2 DWORD)  (u0, space0) UAV 根描述符，用于间接绘制命令缓冲区
		// Para 2: (Type = Root Descriptor,	 2 DWORD)  (u1, space0) UAV 根描述符，用于间接命令缓冲计数器
		// Para 3: (Type = Root Descriptor,	 2 DWORD)  (b0, space0) CBV 根描述符，用于常量缓冲
		// 
		// None Static Sampler	没有静态采样器，计算着色器现在不需要用到
		...
		// 第一个根参数：SRV 根描述符 (m_KeepChunksBuffer)
		...
		// 第二个根参数：UAV 根描述符 (m_IndirectCommandBuffer)
		...
		// 第三个根参数：UAV 根描述符 (m_IndirectCommandCounter)
		...
		// 根签名信息结构体，上限 64 DWORD，静态采样器不占用根签名
		D3D12_ROOT_SIGNATURE_DESC RootSignatureDesc = {};
		RootSignatureDesc.NumParameters = 4;				// 四个根参数
		RootSignatureDesc.pParameters = RootParameters;		// 根参数数组指针
		RootSignatureDesc.NumStaticSamplers = 0;
		RootSignatureDesc.pStaticSamplers = nullptr;
		RootSignatureDesc.Flags = D3D12_ROOT_SIGNATURE_FLAG_NONE;	// 没有特殊标志
	...
	// 创建专门用于 CullingShader 的两个 PSO (m_CullingPSO 与 m_ClearCounterPSO)
	void STEP33_CreateCullingPSO()
	{
		// 计算着色器使用的 PSO 信息结构体，注意类型是 D3D12_COMPUTE_PIPELINE_STATE_DESC
		D3D12_COMPUTE_PIPELINE_STATE_DESC ComputePSODesc = {};
		...
		// 第一次绑定根签名，使用的是上面的 m_ComputeRootSignature
		// 本次设置是将根签名与 PSO 绑定，生成对应版本的根签名适配 PSO，设置渲染管线的输入参数状态
		ComputePSODesc.pRootSignature = m_CullingRootSignature.Get();


		ComPtr<ID3DBlob> ComputeShaderBlob;		// 计算着色器二进制字节码
		ComPtr<ID3DBlob> ErrorBlob;				// 错误字节码

		// 编译 CullingShader 的 CullingCSMain
		D3DCompileFromFile(L"CullingShader.hlsl", nullptr, nullptr, "CullingCSMain", "cs_5_1",
			NULL, NULL, &ComputeShaderBlob, &ErrorBlob);
		if (ErrorBlob)
		{
			OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
			OutputDebugStringA("\n");
		...
		// 创建用于 GPU Culling 的 PSO
		m_D3D12Device->CreateComputePipelineState(&ComputePSODesc, IID_PPV_ARGS(&m_CullingPSO));



		// 编译 CullingShader 的 ClearCounterCSMain
		D3DCompileFromFile(L"CullingShader.hlsl", nullptr, nullptr, "ClearCounterCSMain", "cs_5_1",
			NULL, NULL, &ComputeShaderBlob, &ErrorBlob);
		if (ErrorBlob)
		{
			OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
		...
		// 创建计算着色器专用 PSO
		m_D3D12Device->CreateComputePipelineState(&ComputePSODesc, IID_PPV_ARGS(&m_ClearCounterPSO));
	}


	// 创建用于 CommandBuffer 和 Counter 的四个资源屏障，这四个资源屏障用于 ExecuteIndirect 与 UAV 的转换
	void STEP34_CreateCommandBufferAndCounterBarrier()
	{
		// UAVToIndirect_barrier 屏障：Unordered Access 无序访问状态 -> Indirect Argument 间接命令参数状态
		// 当计算着色器完成 GPU Culling 并填充命令缓冲区时，将资源转换到特定的 Indirect 状态，准备 ExecuteIndirect
		UAVToIndirect_barrier[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		UAVToIndirect_barrier[0].Transition.StateBefore = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
		UAVToIndirect_barrier[0].Transition.StateAfter = D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT;
		UAVToIndirect_barrier[0].Transition.pResource = m_UAVIndirectCommandBuffer_DefaultResource.Get();

		// IndirectToUAV_barrier 屏障：Indirect Argument 间接命令参数状态 -> Unordered Access 无序访问状态
		// 当渲染完成时，将 CommandBuffer 转换回 UAV 状态，供下一帧着色器重新使用，循环往复
		IndirectToUAV_barrier[0].Type = D3D12_RESOURCE_BARRIER_TYPE_TRANSITION;
		IndirectToUAV_barrier[0].Transition.StateBefore = D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT;
		IndirectToUAV_barrier[0].Transition.StateAfter = D3D12_RESOURCE_STATE_UNORDERED_ACCESS;
		IndirectToUAV_barrier[0].Transition.pResource = m_UAVIndirectCommandBuffer_DefaultResource.Get();


		// Counter 和 CommandBuffer 也一样，改变 pResource 就行，下文 Render 可以批量设置
		UAVToIndirect_barrier[1] = UAVToIndirect_barrier[0];
		UAVToIndirect_barrier[1].Transition.pResource = m_UAVIndirectCommandCounter_DefaultResource.Get();
		IndirectToUAV_barrier[1] = IndirectToUAV_barrier[0];
		IndirectToUAV_barrier[1].Transition.pResource = m_UAVIndirectCommandCounter_DefaultResource.Get();
	}
	...
	void UpdateFrameData()
	{
		...
		// 更新已加载区块数量 (CullingShader ConstantBuffer)
		CullingCbufferPointer->KeepChunkCount = ChunkMetaDataHiveBuffer.size();
		// 更新围成视锥体的 6 个平面方程 (CullingShader ConstantBuffer)
		std::vector<XMVECTOR> CameraPlanes = m_FirstCamera.GetFrustumPlanes();
		for (UINT i = 0; i < 6; i++)
		{
			XMStoreFloat4(&CullingCbufferPointer->FrustumPlanes[i], CameraPlanes[i]);
		}
		...
		{
			// 遍历玩家周围所有的可加载区块 (17x17=289 正方体范围)
	...
	void RenderBranch_AppendNewChunkInstance()
	{
		...
		for (UINT MapIndex = 0; MapIndex < ReadyCreateChunkBuffer.size(); MapIndex++)
		{
			...
			// 加载完成，设置这个区块的 AABB 包围盒
			ChunkMetaDataHiveBuffer[ChunkIndex].AABBBox.Center.x = WorldOffsetX + ChunkExtents.x;
			ChunkMetaDataHiveBuffer[ChunkIndex].AABBBox.Center.y = 0 + ChunkExtents.y;
			ChunkMetaDataHiveBuffer[ChunkIndex].AABBBox.Center.z = WorldOffsetZ + ChunkExtents.z;
			ChunkMetaDataHiveBuffer[ChunkIndex].AABBBox.Extents = ChunkExtents;
	...
	// 开启 CullingShader 进行 GPU 视锥剔除，并由 GPU 驱动可见区块的渲染分支
	void RenderBranch_CullingAndRenderChunk()
	{
		// 更新 SRV 已加载区块元数据缓冲，注意每个元素是 CHUNK
		// StructuredBuffer<CHUNK> m_KeepChunksBuffer : register(t0, space0);
		memcpy(CullingTestUploadPointer, &ChunkMetaDataHiveBuffer[0],
			ChunkMetaDataHiveBuffer.size() * sizeof(CHUNK));
		...
		// 先进行 GPU 视锥剔除
		{
			// 设置 CullingRootSignature
			m_CommandList->SetComputeRootSignature(m_CullingRootSignature.Get());

			// 设置 CullingPSO
			m_CommandList->SetPipelineState(m_CullingPSO.Get());

			// 设置第一个根参数：SRV 根描述符 (已加载区块的元数据)
			m_CommandList->SetComputeRootShaderResourceView(0,
				m_SRVKeepChunksBuffer_UploadResource->GetGPUVirtualAddress());

			// 设置第二个根参数：UAV 根描述符 (间接绘制命令缓冲区)
			m_CommandList->SetComputeRootUnorderedAccessView(1,
				m_UAVIndirectCommandBuffer_DefaultResource->GetGPUVirtualAddress());

			// 设置第三个根参数：UAV 根描述符 (绘制命令计数器)
			m_CommandList->SetComputeRootUnorderedAccessView(2,
				m_UAVIndirectCommandCounter_DefaultResource->GetGPUVirtualAddress());

			// 设置第四个根参数：CBV 根描述符 (常量缓冲区)
			m_CommandList->SetComputeRootConstantBufferView(3,
				m_CBVCullingGlobalBuffer_UploadResource->GetGPUVirtualAddress());


			// 一个线程组能调度 64 个子线程，先进行向上取整，计算最少需要的线程组数量
			const UINT ThreadGroupsNum = Ceil(ChunkMetaDataHiveBuffer.size(), 64);
		...
		// 再进行渲染
		{
			...
			// 发起间接命令请求，驱动 GPU 解析并执行 CommandBuffer 中的绘制命令
			// 当 GPU 执行到此命令时，会自动执行 CommandBuffer 的命令，某种意义上的 GPU 自动化渲染
			// 我们只需要绑定 GPU 渲染必要的资源，剩下的 DrawCall 完全交给 GPU，这样就能和 GPU Culling 无缝衔接了
			m_CommandList->ExecuteIndirect(m_CommandSignature.Get(), MaxKeepChunkCount,
				m_UAVIndirectCommandBuffer_DefaultResource.Get(), 0,
				m_UAVIndirectCommandCounter_DefaultResource.Get(), 0);


			// 渲染完成后，将 CommandBuffer 和 Counter 都转换到 Unordered Access 无序访问状态
			// 下面还需要清空 Counter 命令计数器，并准备下一帧的视锥剔除，计算着色器需要 UAV 资源处于正确状态
			m_CommandList->ResourceBarrier(2, IndirectToUAV_barrier);
		...
		// 注意这里！渲染完成，最后需要清空命令缓冲计数器！
		{
			// 设置 CullingRootSignature
			m_CommandList->SetComputeRootSignature(m_CullingRootSignature.Get());

			// 设置 ClearCounterPSO
			m_CommandList->SetPipelineState(m_ClearCounterPSO.Get());
			...
			// 设置第三个根参数：UAV 根描述符 (绘制命令计数器，我们要清空的对象)
			m_CommandList->SetComputeRootUnorderedAccessView(2,
				m_UAVIndirectCommandCounter_DefaultResource->GetGPUVirtualAddress());
	...
	void Render()
	{
		...
		// 每帧都进行 GPU 视锥剔除与渲染
		RenderBranch_CullingAndRenderChunk();
	}
	...
	static void Run(HINSTANCE hins)
	{
		...
		engine.STEP34_CreateCommandBufferAndCounterBarrier();
