...
private:
	...
	// 用于渲染破坏纹理的 PSO，根签名
	// 输入布局与上文的 m_RenderBlockPSO 共用，shader、深度缓冲状态、光栅化深度状态不共用
	...
	// 指向实例资源 m_BlockInstanceResource 的指针
...
public:
	...
	void STEP23_CreatePSO()
	{
		...
		InputElementDesc[5].SemanticName = "ROTATEINDEX";									// 要锚定的语义
		InputElementDesc[5].SemanticIndex = 0;												// 语义索引
		InputElementDesc[5].Format = DXGI_FORMAT_R32_UINT;									// 输入格式
		InputElementDesc[5].InputSlot = 1;													// 输入槽编号
		InputElementDesc[5].AlignedByteOffset = 16;											// 在输入槽中的偏移
		InputElementDesc[5].InputSlotClass = D3D12_INPUT_CLASSIFICATION_PER_INSTANCE_DATA;	// 输入流类型
		InputElementDesc[5].InstanceDataStepRate = 1;										// 实例数据步进率
	...
	void STEP25_CreatePerInstanceBuffer()
	{
		...
		InstanceResourceDesc.Width = MaxInstanceResourceAllocSize * sizeof(BLOCKINSTANCE);
		...
		VertexBufferView[1].SizeInBytes = MaxInstanceResourceAllocSize * sizeof(BLOCKINSTANCE);
	}



	// 将 6 个旋转矩阵复制到常量缓冲的 BlockFaceForwardMatrix 里，后面这个成员不会再变化了
	...
	void UpdateConstantBuffer()
	{
		...
		{
			// 全量更新
			memcpy(m_BlockInstanceMapPointer, &BlockGroup[0], BlockGroup.size() * sizeof(BLOCKINSTANCE));
	...
	UINT ScreenRaycast(UINT* FaceIndex = nullptr)
	{
		...
		*/
		...
		// 最大交互距离 = 7 * 2，方块边长是 2，java 版手长是 4.5 个方块，因为视角原因，我们让手长适当加长一下
		const float MaxInterativeDistance = 14;
		...
		for (UINT i = 0; i < BlockGroup.size(); i++)
		{
			...
			BlockAABBbox.Extents = XMFLOAT3(1, 1, 1);			// 包围盒的 xyz 半边长
