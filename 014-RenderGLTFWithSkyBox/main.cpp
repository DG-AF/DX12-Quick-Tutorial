...
// (14) RenderGLTFWithSkyBox: 加入并渲染 HDR 天空盒，使用 DirectX 12 + Assimp + stb_image 渲染《为美好的生活献上祝福》中的 Q版阿库娅 GLTF 模型
...
// 使用这个宏定义，可以对 stb_image.h 里面的相关组件进行操作
...
// stb 库是由 Sean T. Barrett 开发的一系列单文件轻量级公共领域库，涵盖图像、音频、字体等多媒体处理功能。
// stb 库非常强大，一种功能的所有函数声明和实现全部放在单一头文件下，而且编译速度很快，体积很小，跨平台兼容，而且它还在保持更新
// 我们使用 stb 里面专门用于图像处理的 stb_image.h 对 HDR 环境贴图进行解析；WIC 的功能不变，仍然负责模型纹理的解析
// WIC 出于历史原因没有支持太多的图片解码器 (如果要支持其他的，需要自己写)，而且读一些特殊的 jpg, png 好像也有点问题
// 所以当 WIC 解决不了的情况，可以试试 stb_image.h，可能有奇效
#include<stb_image.h>
...
// 2.项目 -> 属性 -> VC++ 目录 -> 包含目录 -> 添加 stb (或者写 $(SolutionDir)stb)
// 3.项目 -> 属性 -> VC++ 目录 -> 库目录 -> 添加 Assimp/lib (或者写 $(SolutionDir)Assimp/lib )
// 4.将 Assimp/lib 里面的 assimp-vc143-mtd.lib 与 assimp-vc143-mtd.dll 复制到
...
private:
	...
	std::string ModelFileName = "aqua/scene.gltf";		// 模型文件名
	std::string ModelTextureFilePath = "aqua/";			// 模型所在文件夹
	...
	ComPtr<ID3D12PipelineState> m_ModelPSO;			// 渲染模型的 PSO
	...
	std::string HDRMapFilePath = "autumn_field_puresky_1k.hdr";		// HDR 贴图的文件名 (相对路径)

	int HDRMapWidth = 0;								// HDR 贴图的宽度
	int HDRMapHeight = 0;								// HDR 贴图的高度
	int HDRMapChannelsNum = 0;							// HDR 贴图的通道数 (黑白图，灰度图，RGB 三通道，RGBA 四通道 ...)
	BYTE* HDRMapData = nullptr;							// HDR 贴图源数据
	DXGI_FORMAT HDRMapFormat = DXGI_FORMAT_UNKNOWN;		// HDR 贴图的 DXGI 格式
	UINT HDRMapBitsPerPixel = 0;						// HDR 贴图的图像深度 (单位：比特)

	ComPtr<ID3D12Resource> m_HDRTextureMap;				// HDR 贴图的默认堆资源
	ComPtr<ID3D12Resource> m_HDRTextureUploadMap;		// HDR 贴图的上传堆资源

	D3D12_CPU_DESCRIPTOR_HANDLE m_HDRMapSRV_CPUHandle;	// HDR 贴图描述符的 CPU 句柄
	D3D12_GPU_DESCRIPTOR_HANDLE m_HDRMapSRV_GPUHandle;	// HDR 贴图描述符的 GPU 句柄
	...
	ComPtr<ID3D12Resource> m_SkyBoxVertexResource;		// 天空盒顶点资源 (上传堆)
	ComPtr<ID3D12Resource> m_SkyBoxIndexResource;		// 天空盒索引资源 (上传堆)
	D3D12_VERTEX_BUFFER_VIEW SkyBoxVBV = {};			// 天空盒顶点资源描述符
	D3D12_INDEX_BUFFER_VIEW SkyBoxIBV = {};				// 天空盒索引资源描述符

	ComPtr<ID3D12PipelineState> m_SkyBoxPSO;			// 渲染天空盒的 PSO

	ComPtr<ID3D12Resource> m_SkyBoxCBResource;			// 天空盒常量缓冲资源
	XMFLOAT4X4* SkyBoxCBuffer;							// 指向天空盒常量缓冲的指针 (其实就是个 4x4 矩阵)
...
public:
	...
	void STEP1_InitWindow(HINSTANCE hins)
	{
		...
		// 使用上文的窗口类创建窗口
		m_hwnd = CreateWindow(wc.lpszClassName, L"Q版阿库娅 -- Static Pose 静态姿态", WS_SYSMENU | WS_OVERLAPPED,
	...
	void STEP12_AddModelData()
	{
		...
		for (UINT i = 0; i < m_ModelScene->mNumMeshes; i++)
		{
			...
			// 骨骼是网格的核心，因为网格需要依靠骨骼驱动才能完成姿势，骨骼影响了附着在其上的网格，导致网格上顶点位置的改变
			// 骨骼模型的运动是父节点带动子节点，所以上文要用递归的形式计算偏移矩阵，下一节骨骼动画更是如此
			// 然而网格仅仅只是静态绑定在骨骼上，碰到大幅度运动时，关节处必然会出现很明显的裂缝，这类就叫 Rigid Meshes 刚性网格
			// 为了解决这个问题，让皮肤网格跟随骨骼运动而变形，Bone Weight 骨骼权重应运而生，它描述的是网格受骨骼的影响程度
			// 骨骼权重的取值是 [0, 1]，0 表示顶点完全不受该骨骼影响，一个顶点可能受多个骨骼影响，有多个骨骼权重
			// 但无论如何，影响顶点的所有骨骼权重和一定为 1，这样关节移动时，皮肤网格可以自然地拉伸，这类就叫 Skinned Meshes 蒙皮网格
			// 为了简单起见，我们在上文指定 aiProcess_LimitBoneWeights 限制一个顶点最多被 4 个骨骼影响，这样在后面的骨骼动画会很方便


			// 添加骨骼数据，先判断网格有没有绑定骨骼，如果没有就特殊处理 (直接找网格对应的节点)
			if (mesh->HasBones())
			{
				// 遍历影响网格的骨骼，在 Assimp 中，顶点信息与骨骼权重是分开存储的
				for (UINT j = 0; j < mesh->mNumBones; j++)
				{
					// 当前骨骼
					const aiBone* CurrentBone = mesh->mBones[j];
					// 当前骨骼在 TransformGroup 上的索引，可以索引到骨骼对应的变换矩阵 (Bone Space -> Model Space)
					const UINT BoneIndex = BoneNode_IndexGroup[CurrentBone->mName.C_Str()];


					// 注意！Mesh 网格绑定的骨骼，携带的 mOffsetMatrix 成员是该骨骼下 Mesh Space -> Bone Space 的变换矩阵！
					// 不像普通网格直接以骨骼节点建立坐标系那样，绑定骨骼的网格有独立的 Mesh 坐标系
					// 因为蒙皮网格常常受到多个骨骼影响，网格上的顶点常常需要按权重拉伸，所以不同骨骼对一个网格的 mOffsetMatrix 是不同的
					// DirectXMath 的 XMMATRIX 结构体和 Assimp 内置的不同，先手动转换一下
					const aiMatrix4x4 BoneMat = CurrentBone->mOffsetMatrix;

					// 第 j 个骨骼从 Mesh Space -> Bone Space 转换的骨骼偏移矩阵
					XMMATRIX MeshToBoneSpaceMatrix =
						XMMATRIX(
							BoneMat.a1, BoneMat.a2, BoneMat.a3, BoneMat.a4,
							BoneMat.b1, BoneMat.b2, BoneMat.b3, BoneMat.b4,
							BoneMat.c1, BoneMat.c2, BoneMat.c3, BoneMat.c4,
							BoneMat.d1, BoneMat.d2, BoneMat.d3, BoneMat.d4
						);


					// BoneNode_TransformGroup 新索引，用于添加经过 Mesh Space -> Model Space 变换的最终矩阵
					// 这里和上文的 BoneIndex 不同，BoneIndex 索引的矩阵是多个 Mesh 共用的，不能直接改变 BoneIndex 索引对应的矩阵
					...
					// MeshToBoneSpaceMatrix 表示 Mesh Space -> Bone Space 的变换矩阵
					// 两个矩阵相乘就是 Mesh Space -> Model Space，切记，这里矩阵乘法仍然不满足交换律！这两个矩阵的位置不能交换！
					MeshToBoneSpaceMatrix = BoneNode_TransformGroup[BoneIndex] * MeshToBoneSpaceMatrix;

					// 添加新矩阵
					BoneNode_TransformGroup.push_back(MeshToBoneSpaceMatrix);


					// 遍历该骨骼下的所有骨骼权重
					for (UINT k = 0; k < CurrentBone->mNumWeights; k++)
					{
						// 骨骼权重在 VertexGroup 对应的顶点 ID (mVertices)
						const UINT VertexID = CurrentMeshVertexGroupOffset + CurrentBone->mWeights[k].mVertexId;
						// 骨骼权重
						const float Weight = CurrentBone->mWeights[k].mWeight;
						// 当前顶点拥有的骨骼权重数
						const UINT WeightCount = VertexWeightsCountGroup[VertexID];

						// 添加骨骼索引，注意！这里添加的是已完成 Mesh Space -> Model Space 变换矩阵的索引
						VertexGroup[VertexID].BoneIndices[WeightCount] = FinalSkinnedMeshIndex;
						// 添加骨骼权重
						VertexGroup[VertexID].BoneWeights[WeightCount] = Weight;
						// 骨骼权重数 +1
						VertexWeightsCountGroup[VertexID]++;
					}
				}
			}
			else
			{
				// 网格在 TransformGroup 上对应的索引 (_Mesh_i)
				const UINT BoneIndex = BoneNode_IndexGroup[std::string("_Mesh_") + std::to_string(i)];

				// 没有骨骼，每个顶点就按 _Mesh_i 索引到网格对应的偏移矩阵，骨骼权重置 1 (只受一个矩阵的影响)
				for (UINT j = 0; j < mesh->mNumVertices; j++)
				{
					// 骨骼权重在 VertexGroup 对应的顶点 ID (mVertices)
					const UINT VertexID = CurrentMeshVertexGroupOffset + j;
					// 骨骼权重 (该网格上的顶点只受一个节点影响，所以骨骼权重填 1)
					...
					// 添加骨骼索引
					VertexGroup[VertexID].BoneIndices[WeightCount] = BoneIndex;
					// 添加骨骼权重
					VertexGroup[VertexID].BoneWeights[WeightCount] = Weight;
					// 骨骼权重数 +1
					VertexWeightsCountGroup[VertexID]++;
		...
		for (UINT i = 0; i < BoneNode_TransformGroup.size(); i++)
		{
			BoneNode_TransformGroup[i] = XMMatrixTranspose(BoneNode_TransformGroup[i]);
		}
	...
	void CreateSRVHeap()
	{
		...
		SRVHeapDesc.NumDescriptors = MaterialGroup.size() + 1;				// 描述符堆的容量，注意多出来的一个位置要留给后面的天空盒！
	...
	void STEP17_CopyMeshToDefaultResource()
	{
		...
		// 让主线程强制等待命令队列的完成，和上文等待纹理复制完成一个道理，不建议使用
		WaitForSingleObject(RenderEvent, INFINITE);
	}
	...
	// 创建 Constant Buffer Resource 常量缓冲资源，常量缓冲是一块预先分配的高速显存，用于存储每一帧都要变换的资源，这里我们要存储 MVP 和骨骼矩阵
	...
	// 创建根签名，这个根签名可以被 ModelPSO 和 SkyBoxPSO 共用，这个就是根签名的 "一对多" 特性 (根签名的重载/复用)
	// 根签名可以被 PSO 共用的前提是：绑定根签名的 PSO 必须都是根签名的子集。什么意思呢？
	// 根签名有声明的寄存器资源，PSO shader 可以不用；PSO shader 要用的寄存器资源，根签名必须有
	// 例如下面的 Model PSO (shader.hlsl) 要用 b0, t0, s0，根签名声明了，在 API 内部会自动生成一个根签名
	// SkyBox PSO (SkyBoxShader.hlsl) 要用 b1, t0, s1, 根签名也声明了，在 API 内部会自动生成另一个根签名
	// 根签名检测它们的兼容性 (是否正确引用)，检测成功了，就会自动为不同的 PSO 生成不同的根签名版本 (Root Signature Version)
	// 类似于 C++ 编译器的函数重载，根据函数参数的不同，会生成不同的函数版本，用户传递不同的参数，实际上是选择不同版本的同名函数
	// 这样设计可以大大提高 CPU/GPU 资源的利用率，这也是 DX12 相比 DX11 的一个创新设计，根签名就是这么来的
	void STEP19_CreateRootSignature()
	{
		...
		// 第一个根参数：CBV 根描述符 (MVP 与骨骼矩阵)，根描述符是内联描述符，所以下文绑定根参数时，只需要传递常量缓冲资源的地址即可
		...
		// 第二个根参数：CBV 根描述符 (天空盒)
		D3D12_ROOT_DESCRIPTOR SkyboxCBVRoot = {};
		SkyboxCBVRoot.ShaderRegister = 1;	// b1
		SkyboxCBVRoot.RegisterSpace = 0;	// space0

		RootParameters[1].ShaderVisibility = D3D12_SHADER_VISIBILITY_ALL;
		RootParameters[1].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
		RootParameters[1].Descriptor = SkyboxCBVRoot;



		// 第三个根参数：根描述表 (Range: SRV)，模型纹理与天空盒纹理共用
		...
		D3D12_STATIC_SAMPLER_DESC StaticSamplerDesc[2] = {};						// 静态采样器数组，静态采样器不会占用根签名

		// 第一个采样器 (模型纹理)
		StaticSamplerDesc[0].ShaderRegister = 0;									// 要绑定的寄存器槽，对应 s0
		StaticSamplerDesc[0].RegisterSpace = 0;										// 要绑定的寄存器空间，对应 space0
		StaticSamplerDesc[0].ShaderVisibility = D3D12_SHADER_VISIBILITY_PIXEL;		// 静态采样器在着色器中的可见性，这里指定仅在像素着色器可见 (只有像素着色器用到了纹理采样)
		StaticSamplerDesc[0].Filter = D3D12_FILTER_COMPARISON_MIN_MAG_MIP_POINT;	// 纹理过滤类型，这里我们直接选 邻近点采样 就行
		StaticSamplerDesc[0].AddressU = D3D12_TEXTURE_ADDRESS_MODE_BORDER;			// 在 U 方向上的纹理寻址方式
		StaticSamplerDesc[0].AddressV = D3D12_TEXTURE_ADDRESS_MODE_BORDER;			// 在 V 方向上的纹理寻址方式
		StaticSamplerDesc[0].AddressW = D3D12_TEXTURE_ADDRESS_MODE_BORDER;			// 在 W 方向上的纹理寻址方式 (3D 纹理会用到)
		StaticSamplerDesc[0].MinLOD = 0;											// 最小 LOD 细节层次，这里我们默认填 0 就行
		StaticSamplerDesc[0].MaxLOD = D3D12_FLOAT32_MAX;							// 最大 LOD 细节层次，这里我们默认填 D3D12_FLOAT32_MAX (没有 LOD 上限)
		StaticSamplerDesc[0].MipLODBias = 0;										// 基础 Mipmap 采样偏移量，我们这里我们直接填 0 就行
		StaticSamplerDesc[0].MaxAnisotropy = 1;										// 各向异性过滤等级，我们不使用各向异性过滤，需要默认填 1
		StaticSamplerDesc[0].ComparisonFunc = D3D12_COMPARISON_FUNC_NEVER;			// 这个是用于阴影贴图的，我们不需要用它，所以填 D3D12_COMPARISON_FUNC_NEVER
		...
		rootsignatureDesc.NumStaticSamplers = 2;					// 静态采样器数量
	...
	// 创建专门用于渲染模型的 PSO (使用 shader.hlsl)
	void STEP20_CreateModelPSO()
	{
		// PSO 信息结构体
		...
		// 顶点自带颜色 float4 color
		InputElementDesc[2].SemanticName = "COLOR";											// 要锚定的语义
		InputElementDesc[2].SemanticIndex = 0;												// 语义索引
		InputElementDesc[2].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;						// 输入格式
		...
		// 骨骼索引 uint4 BoneIndices
		InputElementDesc[3].SemanticName = "BLENDINDICES";									// 要锚定的语义
		InputElementDesc[3].SemanticIndex = 0;												// 语义索引
		InputElementDesc[3].Format = DXGI_FORMAT_R32G32B32A32_UINT;							// 输入格式
		InputElementDesc[3].InputSlot = 0;													// 输入槽编号
		InputElementDesc[3].AlignedByteOffset = 40;											// 在输入槽中的偏移
		InputElementDesc[3].InputSlotClass = D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA;	// 输入流类型
		InputElementDesc[3].InstanceDataStepRate = 0;										// 实例数据步进率


		// 骨骼权重 float4 BoneWeights
		InputElementDesc[4].SemanticName = "BLENDWEIGHT";									// 要锚定的语义
		InputElementDesc[4].SemanticIndex = 0;												// 语义索引
		InputElementDesc[4].Format = DXGI_FORMAT_R32G32B32A32_FLOAT;						// 输入格式
		InputElementDesc[4].InputSlot = 0;													// 输入槽编号
		InputElementDesc[4].AlignedByteOffset = 56;											// 在输入槽中的偏移
		InputElementDesc[4].InputSlotClass = D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA;	// 输入流类型
		InputElementDesc[4].InstanceDataStepRate = 0;										// 实例数据步进率
		...
		PSODesc.BlendState.RenderTarget[0].BlendEnable = true;		// 开启混合，因为模型用到了自发光和默认纹理
		...
		// 最终创建 PSO 对象
		m_D3D12Device->CreateGraphicsPipelineState(&PSODesc, IID_PPV_ARGS(&m_ModelPSO));
	}
	...
	// 本章我们将正式接触天空盒 (SkyBox)，我们在现实中看到的天空很辽阔，天似穹庐，笼盖四野，凡人之躯不可触其边界，几乎全部笼罩在地面上方
	// 而在 3D 游戏中，天空盒是一个包裹游戏场景的超大立方体，用于渲染和展示远景环境。天空盒除了提供背景视觉，还能提供环境光照

	// 用 stb_image 加载 HDR 环境贴图 (High Dynamic Range Texture/Image，高动态范围贴图/图像)
	// HDR 贴图是一种亮度范围极大，高光和暗部细节能完整保留，可以记录光强信息的纹理图像，常用作天空盒或大型场景
	// 我们需要 HDR 环境贴图渲染天空盒，WIC 没有 HDR 格式的解码器，我们需要用 stb_image 加载贴图文件
	bool STEP21_LoadHDRTextureMap()
	{
		// 根据文件名加载图像到内存，并将像素值转换为 32 位浮点数格式 (stbi_load 是 8 位整数)
		// 第 2-4 个参数都是 output 参数，第五个参数是期望的通道数，0 表示按贴图源通道加载，返回值是解析过的贴图数据
		// 注意这里源数据结果要转换成 BYTE*，因为 float* 和 BYTE* 两者偏移长度不同 (float = 4 BYTE)
		HDRMapData = reinterpret_cast<BYTE*>(stbi_loadf(HDRMapFilePath.c_str(), &HDRMapWidth, &HDRMapHeight, &HDRMapChannelsNum, 0));

		// 如果加载失败
		if (HDRMapData == nullptr)
		{
			std::string err_msg = std::string("无法加载此类型的环境贴图：") + HDRMapFilePath;
			MessageBoxA(NULL, err_msg.c_str(), "错误", MB_OK | MB_ICONERROR);
			return false;
		}

		// 检测 HDR 贴图的通道数，如果是三通道就设置天空盒纹理的 DXGI 纹理格式；如果不是三通道贴图我们是没法用的
		if (HDRMapChannelsNum == 3)
		{
			HDRMapFormat = DXGI_FORMAT_R32G32B32_FLOAT;
		}
		else
		{
			std::string err_msg = std::string("此环境贴图：") + HDRMapFilePath + " 不是三像素通道贴图，加载失败！";
			MessageBoxA(NULL, err_msg.c_str(), "错误", MB_OK | MB_ICONERROR);
			return false;
		}

		// 算出 HDR 贴图的 BPP 图像深度，通道数 * 每个通道数据类型占的字节 * 一个字节占的比特数 (单位：比特)
		HDRMapBitsPerPixel = HDRMapChannelsNum * sizeof(float) * 8;

		return true;
	}


	// 和上面的 CreateUploadAndDefaultResource 一样，算出 HDR 纹理贴图的相关数据，并创建对应的 Upload/Default Resource
	void STEP22_CreateHDRMapResource()
	{
		// 逻辑和上面的 CreateUploadAndDefaultResource，不再赘述。变量重用，注意要替换部分参数！

		BytePerRowSize = HDRMapWidth * HDRMapBitsPerPixel / 8;		// 注意这里！
		TextureSize = BytePerRowSize * HDRMapHeight;				// 注意这里！
		UploadResourceRowSize = Ceil(BytePerRowSize, 256) * 256;
		UploadResourceSize = UploadResourceRowSize * (HDRMapHeight - 1) + BytePerRowSize;	// 注意这里！
		...
		// 创建上传堆资源
		m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE, &UploadResourceDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_HDRTextureUploadMap));
		...
		DefaultResourceDesc.Width = HDRMapWidth;							// 注意这里！
		DefaultResourceDesc.Height = HDRMapHeight;							// 注意这里！
		DefaultResourceDesc.Format = HDRMapFormat;							// 注意这里！
		...
		// 创建默认堆资源
		m_D3D12Device->CreateCommittedResource(&DefaultHeapDesc, D3D12_HEAP_FLAG_NONE, &DefaultResourceDesc,
			D3D12_RESOURCE_STATE_COPY_DEST, nullptr, IID_PPV_ARGS(&m_HDRTextureMap));
	}


	// 和上面的 CopyTextureDataToDefaultResource 一样，两次复制资源到默认堆资源，记录命令并执行
	void STEP23_CopyHDRResource()
	{
		// 用于传递资源的指针
		BYTE* TransferPointer = nullptr;
		// 开启映射
		m_HDRTextureUploadMap->Map(0, nullptr, reinterpret_cast<void**>(&TransferPointer));
		// 逐行复制
		for (UINT i = 0; i < HDRMapHeight; i++)						// 注意这里！
		{
			memcpy(TransferPointer, HDRMapData, BytePerRowSize);	// 注意这里！
			HDRMapData += BytePerRowSize;							// 注意这里！
			TransferPointer += UploadResourceRowSize;
		}
		// 关闭映射
		m_HDRTextureUploadMap->Unmap(0, nullptr);

		D3D12_PLACED_SUBRESOURCE_FOOTPRINT PlacedFootprint = {};				// 资源脚本，用来描述要复制的资源
		D3D12_RESOURCE_DESC DefaultResourceDesc = m_HDRTextureMap->GetDesc();	// 默认堆资源结构体
		...
		DstLocation.Type = D3D12_TEXTURE_COPY_TYPE_SUBRESOURCE_INDEX;		// 纹理复制类型，这里必须指向纹理
		DstLocation.SubresourceIndex = 0;									// 指定要复制的子资源索引
		DstLocation.pResource = m_HDRTextureMap.Get();						// 要复制到的资源

		D3D12_TEXTURE_COPY_LOCATION SrcLocation = {};						// 复制源位置 (上传堆资源) 结构体
		SrcLocation.Type = D3D12_TEXTURE_COPY_TYPE_PLACED_FOOTPRINT;		// 纹理复制类型，这里必须指向缓冲区
		SrcLocation.PlacedFootprint = PlacedFootprint;						// 指定要复制的资源脚本信息
		SrcLocation.pResource = m_HDRTextureUploadMap.Get();				// 被复制数据的缓冲
	...
	// 创建 HDR 纹理资源的 SRV，SRV Heap 的最后一个就是 HDR SRV
	void STEP24_CreateHDRMapSRV()
	{
		// 偏移 CPU 和 GPU 句柄在指定的位置，SRV Heap 最后一个描述符就是 HDR Texture
		m_HDRMapSRV_CPUHandle = m_SRVHeap->GetCPUDescriptorHandleForHeapStart();
		m_HDRMapSRV_CPUHandle.ptr += MaterialGroup.size() * SRVDescriptorSize;
		m_HDRMapSRV_GPUHandle = m_SRVHeap->GetGPUDescriptorHandleForHeapStart();
		m_HDRMapSRV_GPUHandle.ptr += MaterialGroup.size() * SRVDescriptorSize;

		// SRV 描述符信息结构体
		D3D12_SHADER_RESOURCE_VIEW_DESC SRVDescriptorDesc = {};
		SRVDescriptorDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D;
		SRVDescriptorDesc.Format = HDRMapFormat;	// 注意这里！
		SRVDescriptorDesc.Shader4ComponentMapping = D3D12_DEFAULT_SHADER_4_COMPONENT_MAPPING;
		SRVDescriptorDesc.Texture2D.MipLevels = 1;

		// 创建 SRV 描述符
		m_D3D12Device->CreateShaderResourceView(m_HDRTextureMap.Get(), &SRVDescriptorDesc, m_HDRMapSRV_CPUHandle);
	}
	...
	// 创建天空盒资源，并将数据复制到资源中，填写相关的结构体信息
	void STEP25_CreateSkyBoxAndResource()
	{
		// 天空盒顶点数据，这个顶点数据是在 NDC 空间上的，为什么呢？
		// 其实天空盒有两种渲染技术：
		// 传统的天空盒渲染技术，会真的建一个超大的立方体，为了防止玩家真的走出了天空盒的 "边界"，从而露出鸡脚，程序员会让这个立方体跟随玩家中心移动。
		// 可随着现代硬件渲染管线和图形学的进步，早就有一些人对这种技术嗤之以鼻，他们认为这种会徒增计算量，降低渲染效率
		// 聪明的人发现不管如何，图像总是要变换到屏幕上的，也就是说必须要经历 NDC 空间和屏幕空间这两步
		// 既然明的不行，我来暗的！直接让天空盒覆盖整个 NDC 空间，这样玩家无论如何，不就走不出天空盒了吗？
		// 这种方法就叫屏幕空间天空盒渲染 (Screen Space SkyBox)，真的奏效了，直接把 3D 纹理降维成 2D 纹理渲染，而且降低了很多计算量
		// 开发者只需要渲染一个覆盖整个 NDC 空间的正方形，然后通过特殊的纹理采样方法，对天空盒纹理采样就行
		// 我们这里并没有填写纹理 UV 数据，猜猜纹理 UV 从哪里来? (提示: SkyBoxShader.hlsl)
		XMFLOAT4 SkyBoxVertexData[4] =
		{
			{-1.0f, 1.0f, 1.0f, 1.0f},	// 左上角
			{-1.0f, -1.0f, 1.0f, 1.0f},	// 左下角
			{1.0f, 1.0f, 1.0f, 1.0f},	// 右上角
			{1.0f, -1.0f, 1.0f, 1.0f}	// 右下角
		};

		// 天空盒索引数据，注意这里的类型是 UINT32
		UINT32 SkyBoxIndexData[6] = { 0, 1, 2, 1, 2, 3 };

		// 在上传堆的天空盒顶点资源信息结构体，由于顶点数量太少，直接让 GPU 在上传堆访问就行
		D3D12_RESOURCE_DESC SkyBoxVertexUploadDesc = {};
		SkyBoxVertexUploadDesc.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
		SkyBoxVertexUploadDesc.Format = DXGI_FORMAT_UNKNOWN;
		SkyBoxVertexUploadDesc.Width = sizeof(XMFLOAT4) * 4;
		SkyBoxVertexUploadDesc.Height = 1;
		SkyBoxVertexUploadDesc.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
		SkyBoxVertexUploadDesc.DepthOrArraySize = 1;
		SkyBoxVertexUploadDesc.MipLevels = 1;
		SkyBoxVertexUploadDesc.SampleDesc.Count = 1;
		SkyBoxVertexUploadDesc.SampleDesc.Quality = 0;
		...
		m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE,
			&SkyBoxVertexUploadDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_SkyBoxVertexResource));

		// 在上传堆的天空盒索引资源信息结构体
		D3D12_RESOURCE_DESC SkyBoxIndexUploadDesc = SkyBoxVertexUploadDesc;
		SkyBoxIndexUploadDesc.Width = sizeof(UINT32) * 6;

		// 创建天空盒索引资源
		m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE,
			&SkyBoxIndexUploadDesc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_SkyBoxIndexResource));


		// 将内存数据复制到资源中 (CPU Cache -> 共享内存)
		BYTE* TransferPointer = nullptr;
		m_SkyBoxVertexResource->Map(0, nullptr, reinterpret_cast<void**>(&TransferPointer));
		memcpy(TransferPointer, SkyBoxVertexData, sizeof(XMFLOAT4) * 4);
		m_SkyBoxVertexResource->Unmap(0, nullptr);

		m_SkyBoxIndexResource->Map(0, nullptr, reinterpret_cast<void**>(&TransferPointer));
		memcpy(TransferPointer, SkyBoxIndexData, sizeof(UINT32) * 6);
		m_SkyBoxIndexResource->Unmap(0, nullptr);


		// 填写天空盒 IBV 与 VBV
		SkyBoxVBV.BufferLocation = m_SkyBoxVertexResource->GetGPUVirtualAddress();
		SkyBoxVBV.SizeInBytes = sizeof(XMFLOAT4) * 4;
		SkyBoxVBV.StrideInBytes = sizeof(XMFLOAT4);

		SkyBoxIBV.BufferLocation = m_SkyBoxIndexResource->GetGPUVirtualAddress();
		SkyBoxIBV.SizeInBytes = sizeof(UINT32) * 6;
		SkyBoxIBV.Format = DXGI_FORMAT_R32_UINT;
	}


	// 创建专门用于渲染天空盒的 PSO (使用 SkyBoxShader.hlsl)
	void STEP26_CreateSkyBoxPSO()
	{
		// 渲染天空盒的 PSO 信息结构体
		...
		// Input Assembler 输入装配阶段
		D3D12_INPUT_LAYOUT_DESC InputLayoutDesc = {};			// 输入样式信息结构体
		D3D12_INPUT_ELEMENT_DESC InputElementDesc[1] = {};		// 输入元素信息结构体数组
		...
		InputLayoutDesc.NumElements = 1;						// 输入元素个数
		InputLayoutDesc.pInputElementDescs = InputElementDesc;	// 输入元素结构体数组指针
		SkyBoxPSODesc.InputLayout = InputLayoutDesc;			// 设置渲染管线 IA 阶段的输入样式
		...
		// 编译顶点着色器 Vertex Shader
		D3DCompileFromFile(L"SkyBoxShader.hlsl", nullptr, nullptr, "VSMain", "vs_5_1", NULL, NULL, &VertexShaderBlob, &ErrorBlob);
		if (ErrorBlob)		// 如果着色器编译出错，ErrorBlob 可以提供报错信息
		{
			OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
			OutputDebugStringA("\n");
		...
		// 编译像素着色器 Pixel Shader
		D3DCompileFromFile(L"SkyBoxShader.hlsl", nullptr, nullptr, "PSMain", "ps_5_1", NULL, NULL, &PixelShaderBlob, &ErrorBlob);
		if (ErrorBlob)		// 如果着色器编译出错，ErrorBlob 可以提供报错信息
		{
			OutputDebugStringA((const char*)ErrorBlob->GetBufferPointer());
			OutputDebugStringA("\n");
		...
		SkyBoxPSODesc.VS.pShaderBytecode = VertexShaderBlob->GetBufferPointer();	// VS 字节码数据指针
		SkyBoxPSODesc.VS.BytecodeLength = VertexShaderBlob->GetBufferSize();		// VS 字节码数据长度
		SkyBoxPSODesc.PS.pShaderBytecode = PixelShaderBlob->GetBufferPointer();		// PS 字节码数据指针
		SkyBoxPSODesc.PS.BytecodeLength = PixelShaderBlob->GetBufferSize();			// PS 字节码数据长度

		// Rasterizer 光栅化
		SkyBoxPSODesc.RasterizerState.CullMode = D3D12_CULL_MODE_NONE;		// 剔除模式，指定是否开启背面/正面/不剔除，这里不剔除
		SkyBoxPSODesc.RasterizerState.FillMode = D3D12_FILL_MODE_SOLID;		// 填充模式，指定是否开启纯色/线框填充，这里选纯色填充

		// 第一次设置根签名！本次设置是将根签名与 PSO 绑定，设置渲染管线的输入参数状态
		SkyBoxPSODesc.pRootSignature = m_RootSignature.Get();

		// 天空盒不用开启深度测试，混合也不用开，本身就是一个背景材料
		SkyBoxPSODesc.DepthStencilState.DepthEnable = false;
		SkyBoxPSODesc.BlendState.RenderTarget[0].BlendEnable = false;
		...
		// 最终创建 PSO 对象
		m_D3D12Device->CreateGraphicsPipelineState(&SkyBoxPSODesc, IID_PPV_ARGS(&m_SkyBoxPSO));
	}


	// 创建天空盒需要用到的常量缓冲及其资源
	void STEP27_CreateSkyBoxConstantBuffer()
	{
		// 常量资源宽度
		UINT CBufferWidth = Ceil(sizeof(XMFLOAT4X4), 256) * 256;
		...
		// 创建常量缓冲资源
		m_D3D12Device->CreateCommittedResource(&UploadHeapDesc, D3D12_HEAP_FLAG_NONE, &CBVResourceDesc,
			D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&m_SkyBoxCBResource));

		// 开启映射，并获得常量资源的地址，传递给 SkyBoxCBuffer
		m_SkyBoxCBResource->Map(0, nullptr, reinterpret_cast<void**>(&SkyBoxCBuffer));
	}
	...
	// 用于动画的模型骨骼-偏移矩阵组，存储对应骨骼在整个模型的偏移矩阵，大小与 Static Pose 组相同
	// 动画组是在 Static Pose 的基础上进行变换的，绑定骨骼的部分矩阵与 Static Pose 组不同
	std::vector<XMMATRIX> BoneNode_Animation_TransformGroup;

	// 动画模型骨骼- SQT 矩阵组，key 是有动画的骨骼名，value 是 该骨骼的 SQT 动画矩阵
	std::unordered_map<std::string, XMMATRIX> AnimeNode_SQTMatrixGroup;


	// 按下空格切换到动画组时，执行动画渲染准备函数，在 CallBackFunc 会用到
	void BeginAnimationRender()
	{
		if (AnimationIndex != 0)	// 如果不是 Static Pose，就渲染
		{
			...
			// 清空动画矩阵组
			BoneNode_Animation_TransformGroup = std::vector<XMMATRIX>(BoneNode_TransformGroup.size(), XMMatrixIdentity());

			// 清空动画模型骨骼- SQT 矩阵组
			AnimeNode_SQTMatrixGroup.clear();

			// 更换动画，更新标题
			std::string WindowTitle = "Q版阿库娅 -- 动画(" + std::to_string(AnimationIndex) + "): " +
				m_ModelScene->mAnimations[AnimationIndex - 1]->mName.C_Str();
			SetWindowTextA(m_hwnd, WindowTitle.c_str());
		}
		else
		{
			SetWindowText(m_hwnd, L"Q版阿库娅 -- Static Pose 静态姿态");
		}
	}


	// 计算当前时间节点下，模型动画某一个骨骼对应的关键帧与变换矩阵 (包括平移，放缩，旋转)
	// 在 Assimp 的骨骼动画结构中，每一个 channel 就是一个骨骼节点，每个节点的动画信息不同
	// 我们需要计算动画时间 (以动画帧 Tick 为单位，注意是动画帧)，以进行插值实时动态解算
	XMMATRIX CalcTickBoneMatrix(float AnimeTimeInTicks, const aiNodeAnim* AnimeNode)
	{
		XMMATRIX BoneAnime_TranslationMatrix;	// 当前帧平移矩阵
		XMMATRIX BoneAnime_RotationMatrix;		// 当前帧旋转矩阵
		XMMATRIX BoneAnime_ScalingMatrix;		// 当前帧放缩矩阵


		// 求当前帧平移矩阵
		// 如果动画在该节点上只有一帧，就不用插值了
		if (AnimeNode->mNumPositionKeys == 1)
		{
			BoneAnime_TranslationMatrix = XMMatrixTranslation(AnimeNode->mPositionKeys[0].mValue.x,
				AnimeNode->mPositionKeys[0].mValue.y, AnimeNode->mPositionKeys[0].mValue.z);
		}
		else // 不止一帧，就寻找当前时间左右两边对应的两个关键帧
		{
			UINT NextIndex = 0;	// 当前帧右边的关键帧索引 (平移)

			// PositionKeys 存储了动画的关键帧信息，包括关键帧平移信息 mValue，关键帧时间信息 mTime (注意这个 mTime 是以动画帧 Tick 为单位的)
			// 那关键帧之外的其他帧，该如何求解？这个时候，我们需要先找到当前帧左右的两个关键帧，然后再计算进行插值
			for (int i = 1; i < AnimeNode->mNumPositionKeys; i++)
			{
				// 如果找到了比 AnimeTime 大的关键帧时间节点，就返回
				if (AnimeTimeInTicks < AnimeNode->mPositionKeys[i].mTime)
				{
					NextIndex = i;
					break;
				}
			...
			// 左关键帧信息
			XMVECTOR LastTickVec = XMVectorSet(AnimeNode->mPositionKeys[NextIndex - 1].mValue.x,
				AnimeNode->mPositionKeys[NextIndex - 1].mValue.y, AnimeNode->mPositionKeys[NextIndex - 1].mValue.z, 0);
			// 右关键帧信息
			XMVECTOR NextTickVec = XMVectorSet(AnimeNode->mPositionKeys[NextIndex].mValue.x,
				AnimeNode->mPositionKeys[NextIndex].mValue.y, AnimeNode->mPositionKeys[NextIndex].mValue.z, 0);

			// 两关键帧相隔时间 (分母)
			double DeltaTime = AnimeNode->mPositionKeys[NextIndex].mTime - AnimeNode->mPositionKeys[NextIndex - 1].mTime;
			// 当前帧与左关键帧时间之差 (分子)
			double Differ = AnimeTimeInTicks - AnimeNode->mPositionKeys[NextIndex - 1].mTime;
			// 插值因子
			double Factor = Differ / DeltaTime;

			// 插值得到该帧的平移矩阵，下文同理
			BoneAnime_TranslationMatrix = XMMatrixTranslationFromVector(XMVectorLerp(LastTickVec, NextTickVec, Factor));
		}


		// 求当前帧旋转矩阵，注意旋转用的是四元数，插值方式是球面线性插值
		// 模型旋转数据的存储都是四元数，相比 4x4 旋转矩阵，四元数字节数更少，占用的空间小得多
		if (AnimeNode->mNumRotationKeys == 1)
		{
			// 在 DirectXMath 中，存储四元数用的仍然是 XMVECTOR
			XMVECTOR RotationQuaternion = XMVectorSet(AnimeNode->mRotationKeys[0].mValue.x, AnimeNode->mRotationKeys[0].mValue.y,
				AnimeNode->mRotationKeys[0].mValue.z, AnimeNode->mRotationKeys[0].mValue.w);
			// 将四元数转换成旋转矩阵
			BoneAnime_RotationMatrix = XMMatrixRotationQuaternion(XMQuaternionNormalize(RotationQuaternion));
		}
		else
		{
			UINT NextIndex = 0;	// 当前帧右边的关键帧索引 (旋转)

			for (int i = 1; i < AnimeNode->mNumRotationKeys; i++)
			{
				if (AnimeTimeInTicks < AnimeNode->mRotationKeys[i].mTime)
				{
					NextIndex = i;
					break;
				}
			...
			// 左关键帧信息
			XMVECTOR LastTickVec = XMVectorSet(AnimeNode->mRotationKeys[NextIndex - 1].mValue.x,
				AnimeNode->mRotationKeys[NextIndex - 1].mValue.y,
				AnimeNode->mRotationKeys[NextIndex - 1].mValue.z,
				AnimeNode->mRotationKeys[NextIndex - 1].mValue.w);
			// 右关键帧信息
			XMVECTOR NextTickVec = XMVectorSet(AnimeNode->mRotationKeys[NextIndex].mValue.x,
				AnimeNode->mRotationKeys[NextIndex].mValue.y,
				AnimeNode->mRotationKeys[NextIndex].mValue.z,
				AnimeNode->mRotationKeys[NextIndex].mValue.w);

			// 两关键帧相隔时间 (分母)
			double DeltaTime = AnimeNode->mRotationKeys[NextIndex].mTime - AnimeNode->mRotationKeys[NextIndex - 1].mTime;
			// 当前帧与左关键帧时间之差 (分子)
			double Differ = AnimeTimeInTicks - AnimeNode->mRotationKeys[NextIndex - 1].mTime;
			// 插值因子
			double Factor = Differ / DeltaTime;
		...
		{
			BoneAnime_ScalingMatrix = XMMatrixScaling(AnimeNode->mScalingKeys[0].mValue.x,
				AnimeNode->mScalingKeys[0].mValue.y, AnimeNode->mScalingKeys[0].mValue.z);
		}
		else
		{
			UINT NextIndex = 0;	// 当前帧右边的关键帧索引 (放缩)

			for (int i = 1; i < AnimeNode->mNumScalingKeys; i++)
			{
				if (AnimeTimeInTicks < AnimeNode->mScalingKeys[i].mTime)
				{
					NextIndex = i;
					break;
				}
			...
			// 左关键帧信息
			XMVECTOR LastTickVec = XMVectorSet(AnimeNode->mScalingKeys[NextIndex - 1].mValue.x,
				AnimeNode->mScalingKeys[NextIndex - 1].mValue.y, AnimeNode->mScalingKeys[NextIndex - 1].mValue.z, 0);
			// 右关键帧信息
			XMVECTOR NextTickVec = XMVectorSet(AnimeNode->mScalingKeys[NextIndex].mValue.x,
				AnimeNode->mScalingKeys[NextIndex].mValue.y, AnimeNode->mScalingKeys[NextIndex].mValue.z, 0);

			// 两关键帧相隔时间 (分母)
			double DeltaTime = AnimeNode->mScalingKeys[NextIndex].mTime - AnimeNode->mScalingKeys[NextIndex - 1].mTime;
			// 当前帧与左关键帧时间之差 (分子)
			double Differ = AnimeTimeInTicks - AnimeNode->mScalingKeys[NextIndex - 1].mTime;
			// 插值因子
			double Factor = Differ / DeltaTime;

			// 插值得到该帧的放缩矩阵
			BoneAnime_ScalingMatrix = XMMatrixScalingFromVector(XMVectorLerp(LastTickVec, NextTickVec, Factor));
		}


		// 将上述三个矩阵依次相乘，就能得到最终变换矩阵 (SQT 矩阵) 了，注意，顺序 (放缩 -> 旋转 -> 平移) 不能改变！
		return BoneAnime_ScalingMatrix * BoneAnime_RotationMatrix * BoneAnime_TranslationMatrix;
	}


	// 渲染每帧动画前，更新并获取动画节点的的 SQT 矩阵
	void UpdateAnimeNodeMatrix()
	{
		// 更新动画影响的所有节点，SQT 矩阵存储到 AnimeNode_SQTMatrixGroup 中
		for (int i = 0; i < m_ModelScene->mAnimations[AnimationIndex - 1]->mNumChannels; i++)
		{
			// 动画节点 (骨骼)
			const aiNodeAnim* AnimeNode = m_ModelScene->mAnimations[AnimationIndex - 1]->mChannels[i];
			// SQT 矩阵
			const XMMATRIX SQTMatrix = CalcTickBoneMatrix(AnimeTime * AnimeTPS, AnimeNode);
			// 动画节点名称
			const std::string BoneName = AnimeNode->mNodeName.C_Str();
			// 新建键值对，将 SQT 矩阵存储到 AnimeNode_SQTMatrixGroup 中
			AnimeNode_SQTMatrixGroup[BoneName] = SQTMatrix;
		}
	}


	// 递归计算动画某一帧的骨骼矩阵
	void CalcAnimeModelNodeMatrix(const aiNode* node, const XMMATRIX& ParentNodeTransform)
	{
		// 原骨骼在 Static Pose 下对应的骨骼变换矩阵 (assimp)
		const aiMatrix4x4 NodeToFatherMat = node->mTransformation;

		// 原骨骼在 Static Pose 下对应的骨骼变换矩阵
		XMMATRIX CurrentNodeTransform =
			XMMATRIX(
				NodeToFatherMat.a1, NodeToFatherMat.a2, NodeToFatherMat.a3, NodeToFatherMat.a4,
				NodeToFatherMat.b1, NodeToFatherMat.b2, NodeToFatherMat.b3, NodeToFatherMat.b4,
				NodeToFatherMat.c1, NodeToFatherMat.c2, NodeToFatherMat.c3, NodeToFatherMat.c4,
				NodeToFatherMat.d1, NodeToFatherMat.d2, NodeToFatherMat.d3, NodeToFatherMat.d4
			);
		...
		// 如果节点有动画，就使用动画的 SQT 矩阵，Static Pose 的偏移矩阵不用了
		// Assimp 给的 SQT 矩阵指的是 相对父骨骼坐标系下新位置的偏移矩阵
		// 与 Static Pose 的偏移矩阵 是二选一的关系，不用相乘，这一点不要理解错了!
		if (AnimeNode_SQTMatrixGroup.find(BoneName) != AnimeNode_SQTMatrixGroup.end())
		{
			// 注意这里！我们从 Assimp 获取并转化成 DirectX 的 SQT 矩阵是 row-major 行主序的! (DirectXMath 是行主序计算的)
			// 但是 Assimp 自身给的矩阵是 column-major 列主序的!
			// 所以这里要转置才能相乘，不转置切换到动画，模型就会扭曲
			CurrentNodeTransform = XMMatrixTranspose(AnimeNode_SQTMatrixGroup[BoneName]);
		}

		// 骨骼索引
		UINT BoneIndex = BoneNode_IndexGroup[BoneName];

		// 当前骨骼在 Model Space 的绝对偏移矩阵 = 父骨骼偏移矩阵 * 子骨骼偏移矩阵 (Father Space -> Child Space)
		CurrentNodeTransform = ParentNodeTransform * CurrentNodeTransform;
		// 将结果存储到 BoneNode_Animation_TransformGroup 动画骨骼组中
		BoneNode_Animation_TransformGroup[BoneIndex] = CurrentNodeTransform;

		// 递归计算子骨骼矩阵
		for (int i = 0; i < node->mNumChildren; i++)
		{
			CalcAnimeModelNodeMatrix(node->mChildren[i], CurrentNodeTransform);
		}
	}


	// 变换得到 Mesh 网格最终的动画矩阵
	void TransformMeshToBoneFinalMatrix()
	{
		for (UINT i = 0; i < m_ModelScene->mNumMeshes; i++)
		{
			// 模型网格
			...
			for (int j = 0; j < mesh->mNumBones; j++)
			{
				...
				// 骨骼索引
				const UINT BoneIndex = BoneNode_IndexGroup[CurrentBone->mName.C_Str()];
				// Mesh 从 Static Pose 下的模型空间到对应骨骼空间的变换矩阵 (Assimp Offset Matrix)
				const aiMatrix4x4 BoneMat = CurrentBone->mOffsetMatrix;

				// Mesh 从 Static Pose 下的模型空间到对应骨骼空间的变换矩阵 (Offset Matrix)
				XMMATRIX MeshToBoneSpaceMatrix =
					XMMATRIX(
						BoneMat.a1, BoneMat.a2, BoneMat.a3, BoneMat.a4,
						BoneMat.b1, BoneMat.b2, BoneMat.b3, BoneMat.b4,
				...
				// 最终变换矩阵，这个矩阵会存储到 cbuffer 中，进行后续的骨骼权重混合
				// 网格相对骨骼的偏移矩阵 = 骨骼偏移矩阵 * 网格到骨骼的矩阵 (Mesh Space -> Bone Space)
				BoneNode_Animation_TransformGroup[FinalSkinnedMeshIndex] =
					BoneNode_Animation_TransformGroup[BoneIndex] * MeshToBoneSpaceMatrix;
			}
		}
	...
	// 更新常量缓冲区，更新每帧都需要用的常量数据
	void UpdateConstantBuffer()
	{
		// 将更新后的矩阵，存储到共享内存上的常量缓冲，这样 GPU 就可以访问到 MVP 矩阵了
		XMStoreFloat4x4(&MVPBuffer->MVPMatrix, m_FirstCamera.GetMVPMatrix());

		// 更新 SkyBoxShader.hlsl 中的逆视图投影矩阵，注意相乘顺序不能交换！
		XMStoreFloat4x4(SkyBoxCBuffer, m_FirstCamera.GetInverseProjectionMatrix() * m_FirstCamera.GetInverseViewMatrix());

		// 如果 AnimationIndex = 0，就渲染 Static Pose 静态姿势
		if (AnimationIndex == 0)
		{
			// 更新在 Static Pose 下的骨骼偏移矩阵组
			// 这里可以直接 memcpy 进行复制，因为 XMMATRIX 和 XMFLOAT4X4 结构相同，占用内存也相同
			memcpy(&MVPBuffer->BoneTransformMatrix[0], &BoneNode_TransformGroup[0],
				BoneNode_TransformGroup.size() * sizeof(XMFLOAT4X4));
		}
		else // 渲染动画
		{
			// 获取当前时间
			...
			// 更新插值变换后的骨骼偏移矩阵组
			// 这里可以直接 memcpy 进行复制，因为 XMMATRIX 和 XMFLOAT4X4 结构相同，占用内存也相同
			memcpy(&MVPBuffer->BoneTransformMatrix[0], &BoneNode_Animation_TransformGroup[0],
				BoneNode_Animation_TransformGroup.size() * sizeof(XMFLOAT4X4));
		}
	}
	...
	void Render()
	{
		...
		// 注意这里的根签名可以被下面的两个 PSO 共用，所以我们不需要再额外创建一个 PSO 了
		...
		// 设置常量缓冲 (第二个根参数)
		m_CommandList->SetGraphicsRootConstantBufferView(1, m_SkyBoxCBResource->GetGPUVirtualAddress());

		// 设置图元拓扑 (输入装配阶段)，我们这里设置三角形列表
		m_CommandList->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_TRIANGLELIST);

		// 设置 VBV，IBV 描述符
		m_CommandList->IASetVertexBuffers(0, 1, &SkyBoxVBV);
		m_CommandList->IASetIndexBuffer(&SkyBoxIBV);

		// 设置天空盒纹理 (第三个根参数)，这个根参数可以被下面的 Model PSO 共用
		m_CommandList->SetGraphicsRootDescriptorTable(2, m_HDRMapSRV_GPUHandle);
		...
		// 设置渲染管线状态，接下来渲染模型
		m_CommandList->SetPipelineState(m_ModelPSO.Get());
		...
		{
			// 设置纹理 SRV (第三个根参数)
			m_CommandList->SetGraphicsRootDescriptorTable(2, MaterialGroup[Mesh.MaterialIndex].GPUHandle);
		...
		switch (msg)
		{
			...
			case WM_CHAR:	// 获取键盘产生的字符消息，TranslateMessage 会将虚拟键码翻译成字符码，同时产生 WM_CHAR 消息
			{
				switch (wParam)		// wParam 是按键对应的字符 ASCII 码
				{
					...
					case ' ':	// 空格
						if (m_ModelScene->HasAnimations())	// 如果有动画，就累增 AnimationIndex，准备渲染新动画
						{
							AnimationIndex++;
							AnimationIndex %= (AnimationCount + 1);
							BeginAnimationRender();			// 在这里准备动画渲染，进行一些初始化设置
						}
						break;
				}
			}
